...
function ReviewCard({
  ...
  const pricingLabel = () => {
    ...
    if (agent.pricing_model === "per_call")     return `$${(agent.price_per_call ?? 0).toFixed(3)} / call`
    if (agent.pricing_model === "subscription") return `$${(agent.subscription_price_monthly ?? 0).toFixed(2)} / mo`
    if (agent.pricing_model === "freemium")     return `Freemium · $${(agent.price_per_call ?? 0).toFixed(3)} / call`
  ...
  return (
    ...
    >
      ...
      <div className="px-5 py-4 flex items-start gap-4">
        ...
        <div className="flex-1 min-w-0">
          <div className="flex items-center gap-2 flex-wrap mb-1">
            <h3 className="font-semibold text-sm text-zinc-900">{agent.name}</h3>
            <StatusBadge status={agent.status} />
            <span className="text-[10px] font-medium text-zinc-400 capitalize px-2 py-0.5 bg-zinc-50 rounded-full border border-zinc-100">
          ...
          <div className="flex items-center gap-3 mt-2 flex-wrap">
            <span className="text-[11px] text-zinc-400 flex items-center gap-1">
              ...
              {agent.profiles?.full_name || "Unknown"} · {agent.profiles?.email || ""}
            </span>
            <span className="text-[11px] text-zinc-400 flex items-center gap-1">
              <Calendar className="h-3 w-3" />
              Submitted {formatRelativeTime(agent.created_at)}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2 flex-shrink-0">
          <button
            ...
            className="p-2 rounded-xl text-zinc-400 hover:text-zinc-700 hover:bg-zinc-50 transition-colors"
            title={expanded ? "Collapse" : "Expand details"}
          >
            {expanded ? <ChevronUp className="h-4 w-4" /> : <ChevronDown className="h-4 w-4" />}
          </button>
          <Link href={`/marketplace/${agent.id}`} target="_blank">
            <button className="p-2 rounded-xl text-zinc-400 hover:text-zinc-700 hover:bg-zinc-50 transition-colors" title="Preview in marketplace">
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-xs font-semibold text-green-600 bg-green-50 border border-green-100 hover:bg-green-100 px-3 py-1.5 rounded-xl transition-colors disabled:opacity-50"
          >
          ...
          <button
            ...
            disabled={loadingAction !== null}
            className="flex items-center gap-1.5 text-xs font-semibold text-red-600 bg-red-50 border border-red-100 hover:bg-red-100 px-3 py-1.5 rounded-xl transition-colors disabled:opacity-50"
          >
      ...
      {/* Rejection form */}
      {rejecting && (
        <div className="px-5 pb-4 border-t border-zinc-50 pt-3 space-y-2">
          <p className="text-xs font-semibold text-zinc-600">Rejection reason <span className="text-red-500">*</span></p>
          <Textarea
            value={rejectReason}
            onChange={e => setRejectReason(e.target.value)}
            placeholder="Explain why this submission is being rejected — this message will be sent to the seller…"
            rows={3}
            className="text-xs rounded-xl border-zinc-200 resize-none"
          />
          <div className="flex gap-2">
            <button
              onClick={handleReject}
              disabled={loadingAction !== null || !rejectReason.trim()}
              className="text-xs font-semibold text-red-600 bg-red-50 border border-red-100 hover:bg-red-100 px-4 py-2 rounded-xl transition-colors disabled:opacity-40"
            >
              {loadingAction === "reject" ? "Rejecting…" : "Confirm Rejection"}
            </button>
            <button
              onClick={() => { setRejecting(false); setRejectReason("") }}
              className="text-xs font-semibold text-zinc-500 hover:text-zinc-900 px-4 py-2 rounded-xl transition-colors"
            >
              Cancel
            </button>
          </div>
      ...
      {expanded && (
        <div className="border-t border-zinc-50 px-5 py-4 space-y-4 bg-zinc-50/40">
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <div className="bg-white rounded-xl border border-zinc-100 px-3 py-2.5">
              <p className="text-[10px] text-zinc-400 font-semibold uppercase tracking-wider mb-0.5 flex items-center gap-1">
                <Cpu className="h-3 w-3" /> Model
              </p>
              <p className="text-xs font-semibold text-zinc-700 truncate">{agent.model_name}</p>
            </div>
            <div className="bg-white rounded-xl border border-zinc-100 px-3 py-2.5">
              <p className="text-[10px] text-zinc-400 font-semibold uppercase tracking-wider mb-0.5 flex items-center gap-1">
                <DollarSign className="h-3 w-3" /> Pricing
              </p>
              <p className="text-xs font-semibold text-zinc-700">{pricingLabel()}</p>
            </div>
            <div className="bg-white rounded-xl border border-zinc-100 px-3 py-2.5">
              <p className="text-[10px] text-zinc-400 font-semibold uppercase tracking-wider mb-0.5">Temp / Tokens</p>
              <p className="text-xs font-semibold text-zinc-700">{agent.temperature} · {agent.max_tokens?.toLocaleString()}</p>
            </div>
            <div className="bg-white rounded-xl border border-zinc-100 px-3 py-2.5">
              <p className="text-[10px] text-zinc-400 font-semibold uppercase tracking-wider mb-0.5 flex items-center gap-1">
                <Hash className="h-3 w-3" /> Pricing model
              </p>
              <p className="text-xs font-semibold text-zinc-700 capitalize">{agent.pricing_model?.replace(/_/g, " ")}</p>
            </div>
          </div>
          ...
          {(agent.tags?.length > 0 || agent.capability_tags?.length > 0) && (
            <div className="space-y-1.5">
              <p className="text-[10px] text-zinc-400 font-semibold uppercase tracking-wider flex items-center gap-1">
                <Tag className="h-3 w-3" /> Tags
              </p>
              <div className="flex flex-wrap gap-1.5">
                {[...(agent.tags ?? []), ...(agent.capability_tags ?? [])].map(tag => (
                  <span key={tag} className="text-[11px] font-medium px-2 py-0.5 bg-white border border-zinc-100 rounded-full text-zinc-600">
                    {tag}
                  </span>
                ))}
              </div>
          ...
          <div className="space-y-1.5">
            <p className="text-[10px] text-zinc-400 font-semibold uppercase tracking-wider flex items-center gap-1">
              <MessageSquare className="h-3 w-3" /> Full description
            </p>
            <p className="text-xs text-zinc-600 leading-relaxed bg-white border border-zinc-100 rounded-xl px-4 py-3">
              {agent.description}
            </p>
          </div>
          ...
          <div className="flex items-center gap-3 bg-white border border-zinc-100 rounded-xl px-4 py-3">
            <Avatar className="h-8 w-8">
              <AvatarFallback className="text-xs bg-primary/8 text-primary">
                {getInitials(agent.profiles?.full_name || agent.profiles?.email || "?")}
              </AvatarFallback>
            </Avatar>
            <div>
...
export function AdminClient({
  stats,
  recentAgents:  initAgents,
  recentUsers:   initUsers,
  flaggedAttempts,
  pendingReviews: initPendingReviews,
}: Props) {
  ...
  const approveReview = useCallback(async (id: string) => {
    const res = await fetch("/api/admin/agents", {
      ...
      body:    JSON.stringify({ agent_id: id, action: "approve" }),
    })
  ...
  const rejectAgent = async (id: string) => {
    const res = await fetch("/api/admin/agents", {
      method:  "PATCH",
      headers: { "Content-Type": "application/json" },
      body:    JSON.stringify({ agent_id: id, action: "suspend", reason: "Admin moderation" }),
    })
    const json = await res.json()
    if (!res.ok) { toast.error(json.error); return }
  ...
  const METRICS = [
    ...
    { label: "Total Agents",     value: formatNumber(stats.totalAgents),     icon: Bot,          color: "text-violet-600", bg: "bg-violet-50",  sub: `${agentTabPending} pending review` },
    ...
    <div className="flex min-h-screen bg-zinc-50">
      ...
      <main className="flex-1 overflow-auto bg-white">
        <div className="max-w-7xl mx-auto px-6 py-8 space-y-8">
          ...
          {/* ── Tabs ───────────────────────────────────────────────────── */}
          <Tabs defaultValue={pendingCount > 0 ? "reviews" : "agents"}>
            <TabsList className="bg-zinc-50 border border-zinc-100 p-1 rounded-xl mb-5 flex-wrap h-auto gap-1">

              {/* Review Submissions tab — primary action */}
            ...
            <TabsContent value="reviews" className="space-y-4">
              {pendingCount === 0 ? (
                <div className="bg-white border border-zinc-100 rounded-2xl flex flex-col items-center justify-center py-16 text-center"
                  ...
                  <p className="text-xs text-zinc-400 max-w-xs">
                    No agent submissions waiting for review. New submissions will appear here automatically.
                  </p>
                </div>
              ) : (
                <>
                  <div className="flex items-center gap-2 bg-amber-50 border border-amber-100 rounded-xl px-4 py-3 text-sm text-amber-700">
                    ...
                    <span>
                      <strong>{pendingCount}</strong> agent{pendingCount > 1 ? "s" : ""} submitted for review.
                      Sellers are notified when you approve or reject. Rejected agents return to <em>draft</em> status.
                    </span>
                  </div>
                  <div className="space-y-3">
                    {pendingReviews.map(agent => (
                      <ReviewCard
                        key={agent.id}
                        agent={agent}
                  ...
                  {filteredAgents.length === 0 ? (
                    ...
                    <div key={agent.id} className="grid grid-cols-12 gap-3 px-5 py-3.5 items-center hover:bg-zinc-50/50 transition-colors">
                      ...
                      <div className="col-span-2">
                        <span className="text-xs text-zinc-500 capitalize">{agent.category?.replace(/_/g, " ")}</span>
                      </div>
                  ...
                  {filteredUsers.length === 0 ? (
                    ...
                    <div key={u.id} className={cn("grid grid-cols-12 gap-3 px-5 py-3.5 items-center hover:bg-zinc-50/50 transition-colors",
                      ...
                      <div className="col-span-2">
                        ...
                        {u.is_banned && (
                          <span className="ml-1 text-[10px] font-bold px-2 py-0.5 rounded-full bg-red-50 text-red-600">banned</span>
                        )}
                      </div>
                      <div className="col-span-2">
            ...
            <TabsContent value="security" className="space-y-4">
              <div className="bg-amber-50 border border-amber-100 rounded-xl px-4 py-3 text-sm text-amber-700 flex items-center gap-2">
                ...
                Prompt injection attempts are logged here. <strong className="mx-1">Blocked</strong> = immediately rejected.
                <strong className="mx-1">Flagged</strong> = suspicious but allowed, logged for review.
              </div>
              <div className="bg-white border border-zinc-100 rounded-2xl overflow-hidden" style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                <div className="grid grid-cols-12 gap-3 px-5 py-2.5 border-b border-zinc-50 text-[10px] font-semibold text-zinc-400 uppercase tracking-wider">
                ...
                <div className="divide-y divide-zinc-50">
                  {flaggedAttempts.length === 0 ? (
                    <div className="text-center py-10 text-sm text-zinc-400 flex flex-col items-center gap-2">
                      <Shield className="h-6 w-6 text-green-400" />
                      No flagged attempts — platform is clean ✓
                    </div>
