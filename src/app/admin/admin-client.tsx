...
interface Credits {
  ...
  hourlyBurn: { hour: number; cost: number }[]
  chartData?: { label: string; cost: number }[]
  alerts: { type: string; message: string; severity: string }[]
  rangeLabel?: string; trendPct?: number
}
interface QueueData {
...
interface Economics {
  ...
  rangeRevenue?: number; range?: string
}
interface Routing {
...
type BurnRange    = "1h" | "1d" | "7d" | "15d"
type RevenueRange = "1d" | "7d" | "15d" | "30d"
type AnyRange     = BurnRange | RevenueRange

const RANGE_LABELS: Record<AnyRange, string> = {
  "1h":"Last hour", "1d":"Today", "7d":"Last 7d", "15d":"Last 15d", "30d":"Last 30d"
}

/** Pill-group range picker — used inline inside chart card headers */
function RangeToggle<T extends string>({
  ...
  return (
    <div className="flex items-center gap-0.5 bg-zinc-100 rounded-lg p-0.5 ml-auto">
      {options.map(r => (
        <button
          key={r}
          onClick={() => onChange(r)}
...
/** Spinner shown on chart while refetching */
...
function SectionCard({ title, sub, icon: Icon, children, className, right }: {
  title: string; sub?: string; icon?: any; children: React.ReactNode; className?: string; right?: React.ReactNode
}) {
  return (
...
function CommandCenter({ stats, execHealth, credits, queue, pendingCount, flaggedCount }: {
  ...
  // ── Burn chart state ──────────────────────────────────────────────────────
  const [burnRange,     setBurnRange]     = useState<BurnRange>("1d")
  const [burnData,      setBurnData]      = useState<{label:string;cost:number}[]>(
    credits?.hourlyBurn?.map(h => ({ label: `${h.hour}:00`, cost: h.cost })) ?? []
  )
  const [burnFetching,  setBurnFetching]  = useState(false)

  useEffect(() => {
    if (burnRange === "1d" && credits?.hourlyBurn) {
      setBurnData(credits.hourlyBurn.map(h => ({ label: `${h.hour}:00`, cost: h.cost })))
      return
    }
    setBurnFetching(true)
    fetch(`/api/admin/credits?range=${burnRange}`)
      .then(r => r.ok ? r.json() : null)
      .then(d => { if (d?.chartData) setBurnData(d.chartData); setBurnFetching(false) })
      .catch(() => setBurnFetching(false))
  }, [burnRange]) // eslint-disable-line react-hooks/exhaustive-deps

  // ── Revenue chart state ───────────────────────────────────────────────────
  const [revRange,    setRevRange]    = useState<RevenueRange>("30d")
  const [revData,     setRevData]     = useState<{label:string;revenue:number}[]>(
    credits ? [] : []
  )
  ...
  useEffect(() => {
    setRevFetching(true)
    fetch(`/api/admin/economics?range=${revRange}`)
      .then(r => r.ok ? r.json() : null)
      .then(d => { if (d?.chartData) setRevData(d.chartData); setRevFetching(false) })
      .catch(() => setRevFetching(false))
  }, [revRange])
  ...
  const heroCards = [
    ...
    { label:"Credit Burn Today", value:`$${burn.toFixed(4)}`,                   sub:`$${(credits?.consumedThisHour??0).toFixed(4)} last hr`, icon:Flame,  color:"text-orange-600", bg:"bg-orange-50", alert:burn>20 },
  ...
  const maxBurn  = Math.max(...burnData.map(d => d.cost), 0.0001)
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {/* Burn chart with range toggle */}
        <SectionCard
          title="Credit Burn"
          sub={RANGE_LABELS[burnRange]}
          icon={Flame}
          right={
            <RangeToggle<BurnRange>
              value={burnRange}
              onChange={setBurnRange}
              options={["1h","1d","7d","15d"] as const}
            />
          }
        >
          <div className="px-2 pt-4 pb-2 relative">
            {burnFetching && <ChartSpinner />}
            <ResponsiveContainer width="100%" height={140}>
              <BarChart data={burnData} margin={{top:0,right:12,bottom:0,left:0}}>
                ...
                <XAxis dataKey="label" tick={{fontSize:9,fill:"#a1a1aa"}} tickLine={false} axisLine={false}
                  interval={burnRange==="1h"?1:burnRange==="1d"?3:0}/>
                <YAxis tick={{fontSize:9,fill:"#a1a1aa"}} tickLine={false} axisLine={false}
                  tickFormatter={v=>`$${v.toFixed(3)}`} width={46}/>
                <Tooltip content={<CustomTooltip prefix="$"/>}/>
                <Bar dataKey="cost" radius={[3,3,0,0]}>
                  {burnData.map((d,i) => (
                    <Cell key={i} fill={d.cost>=maxBurn*0.8?CHART_COLORS.red:d.cost>=maxBurn*0.5?CHART_COLORS.amber:CHART_COLORS.orange}/>
                  ))}
                </Bar>
              </BarChart>
            </ResponsiveContainer>
              ...
              : feedData.map(e => (
                <div key={e.id} className="flex items-center gap-3 px-5 py-2.5">
                  <div className={cn("w-1.5 h-1.5 rounded-full flex-shrink-0", ({
                    success:"bg-emerald-400", failed:"bg-red-400",
                    queued:"bg-amber-400", running:"bg-blue-400", timeout:"bg-orange-400",
                  } as any)[e.status] ?? "bg-zinc-300")} />
      ...
      {/* Revenue chart + queue + top burners */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4">
        {/* Revenue chart with range toggle */}
        <div className="lg:col-span-2">
          <SectionCard
            title="Revenue"
            sub={RANGE_LABELS[revRange]}
            icon={TrendingUp}
            right={
              <RangeToggle<RevenueRange>
                value={revRange}
                onChange={setRevRange}
                options={["1d","7d","15d","30d"] as const}
              />
            }
          >
            <div className="px-2 pt-4 pb-3 relative">
              {revFetching && <ChartSpinner />}
              <ResponsiveContainer width="100%" height={120}>
                <AreaChart data={revData} margin={{top:0,right:12,bottom:0,left:0}}>
                  <defs>
                  ...
                  <CartesianGrid stroke="#f4f4f5" strokeDasharray="2 4" vertical={false}/>
                  <XAxis dataKey="label" tick={{fontSize:9,fill:"#a1a1aa"}} tickLine={false} axisLine={false}
                    interval={revRange==="30d"?4:revRange==="15d"?2:revRange==="7d"?0:3}/>
                  <YAxis tick={{fontSize:9,fill:"#a1a1aa"}} tickLine={false} axisLine={false}
                    tickFormatter={v=>`$${v}`} width={40}/>
                  <Tooltip content={<CustomTooltip prefix="$"/>}/>
                  <Area type="monotone" dataKey="revenue" stroke={CHART_COLORS.primary} strokeWidth={1.5}
                    fill="url(#ccRevGrad)" dot={false}/>
                </AreaChart>
              </ResponsiveContainer>
            </div>
          </SectionCard>
...
function CreditMonitor({ credits }: { credits: Credits | null }) {
  ...
  const [range,      setRange]      = useState<BurnRange>("1d")
  const [chartData,  setChartData]  = useState<{label:string;cost:number}[]>(
    credits.hourlyBurn?.map(h => ({ label: `${h.hour}:00`, cost: h.cost })) ?? []
  )
  const [kpis,       setKpis]       = useState({
    consumed:  credits.consumedToday,
    refunded:  credits.refundedToday,
    failed:    credits.failedToday,
    trendPct:  0,
    rangeLabel:"Today",
  })
  ...
  useEffect(() => {
    if (range === "1d") {
      setChartData(credits.hourlyBurn?.map(h => ({ label: `${h.hour}:00`, cost: h.cost })) ?? [])
      setKpis({ consumed:credits.consumedToday, refunded:credits.refundedToday, failed:credits.failedToday, trendPct:0, rangeLabel:"Today" })
      return
    }
    setFetching(true)
    fetch(`/api/admin/credits?range=${range}`)
      .then(r => r.ok ? r.json() : null)
      .then(d => {
        if (!d) return
        if (d.chartData)  setChartData(d.chartData)
        setKpis({
          consumed:   d.consumed   ?? d.consumedToday ?? 0,
          refunded:   d.refunded   ?? d.refundedToday ?? 0,
          failed:     d.failed     ?? d.failedToday   ?? 0,
          trendPct:   d.trendPct   ?? 0,
          rangeLabel: d.rangeLabel ?? RANGE_LABELS[range],
        })
        setFetching(false)
      })
  ...
  const maxBurn = Math.max(...chartData.map(d => d.cost), 0.0001)
  const trendUp = kpis.trendPct > 0

  return (
    <div className="space-y-5">
      {/* KPIs — update with range */}
      <div className="grid grid-cols-2 lg:grid-cols-4 gap-3">
        <KpiCard
          label={`Consumed — ${kpis.rangeLabel}`}
          value={`$${kpis.consumed.toFixed(4)}`}
          sub={kpis.trendPct !== 0 ? `${trendUp?"+":""}${kpis.trendPct.toFixed(1)}% vs prev period` : "vs previous period"}
          icon={Flame}
          color="text-orange-600" bg="bg-orange-50"
          trend={kpis.trendPct>5?"up":kpis.trendPct<-5?"down":null}
        />
        <KpiCard
          label="Consumed — Last Hour"
          value={`$${credits.consumedThisHour.toFixed(4)}`}
          sub="rolling 60 min"
          icon={Clock}
          color="text-amber-600" bg="bg-amber-50"
          alert={credits.consumedThisHour > 10}
        />
        <KpiCard
          label={`Refunded — ${kpis.rangeLabel}`}
          value={`$${kpis.refunded.toFixed(4)}`}
          sub="from failed executions"
          icon={CreditCard}
          color="text-blue-600" bg="bg-blue-50"
        />
        <KpiCard
          label={`Failed — ${kpis.rangeLabel}`}
          value={String(kpis.failed)}
          sub="executions failed"
          icon={XCircle}
          color={kpis.failed>50?"text-red-600":"text-zinc-600"}
          bg={kpis.failed>50?"bg-red-50":"bg-zinc-50"}
        />
      </div>
      ...
      >
        <div className="px-4 pt-5 pb-3 relative">
          {fetching && <ChartSpinner />}
          <ResponsiveContainer width="100%" height={180}>
            <BarChart data={chartData} margin={{top:0,right:8,bottom:0,left:0}}>
              <CartesianGrid stroke="#f4f4f5" strokeDasharray="2 4" vertical={false}/>
              <XAxis dataKey="label" tick={{fontSize:9,fill:"#a1a1aa"}} tickLine={false} axisLine={false}
                interval={range==="1h"?1:range==="1d"?2:0}/>
              <YAxis tick={{fontSize:9,fill:"#a1a1aa"}} tickLine={false} axisLine={false}
                tickFormatter={v=>`$${v.toFixed(3)}`} width={50}/>
              <Tooltip content={<CustomTooltip prefix="$"/>}/>
              <Bar dataKey="cost" radius={[3,3,0,0]}>
                {chartData.map((d,i) => (
                  <Cell key={i} fill={d.cost>=maxBurn*0.8?CHART_COLORS.red:d.cost>=maxBurn*0.5?CHART_COLORS.amber:CHART_COLORS.orange}/>
      ...
      {/* Tables */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <SectionCard title="Top Burn Users" sub={kpis.rangeLabel} icon={Target}>
          <div className="divide-y divide-zinc-50">
        ...
        <SectionCard title="Top Burn Agents" sub={kpis.rangeLabel} icon={Bot}>
          <div className="divide-y divide-zinc-50">
...
function EconomicsPanel({ econ }: { econ: Economics | null }) {
  ...
  const [revRange,    setRevRange]    = useState<RevenueRange>("30d")
  const [revData,     setRevData]     = useState<{label:string;revenue:number}[]>(
    econ.chartData ?? econ.dailyRevenue.map(d => ({ label: d.date, revenue: d.revenue }))
  )
  const [revFetching, setRevFetching] = useState(false)
  const [revTotal,    setRevTotal]    = useState<number|null>(null)

  useEffect(() => {
    if (revRange === "30d" && (econ.chartData || econ.dailyRevenue.length)) {
      setRevData(econ.chartData ?? econ.dailyRevenue.map(d => ({ label: d.date, revenue: d.revenue })))
      setRevTotal(null)
      return
    }
    setRevFetching(true)
    fetch(`/api/admin/economics?range=${revRange}`)
      .then(r => r.ok ? r.json() : null)
      .then(d => {
        if (d?.chartData) setRevData(d.chartData)
        if (typeof d?.rangeRevenue === "number") setRevTotal(d.rangeRevenue)
        setRevFetching(false)
      })
      .catch(() => setRevFetching(false))
  }, [revRange]) // eslint-disable-line react-hooks/exhaustive-deps
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <SectionCard
          title="Revenue"
          sub={revTotal != null ? `${formatCurrency(revTotal)} — ${RANGE_LABELS[revRange]}` : RANGE_LABELS[revRange]}
          icon={TrendingUp}
          right={
            <RangeToggle<RevenueRange>
              value={revRange}
        ...
        >
          <div className="px-2 pt-5 pb-3 relative">
            {revFetching && <ChartSpinner />}
            <ResponsiveContainer width="100%" height={160}>
              <AreaChart data={revData} margin={{top:0,right:16,bottom:0,left:0}}>
                <defs>
                  <linearGradient id="econRevGrad" x1="0" y1="0" x2="0" y2="1">
                    <stop offset="0%" stopColor={CHART_COLORS.primary} stopOpacity={0.2}/>
                    <stop offset="100%" stopColor={CHART_COLORS.primary} stopOpacity={0}/>
                  </linearGradient>
                </defs>
                ...
                <Area type="monotone" dataKey="revenue" stroke={CHART_COLORS.primary} strokeWidth={2}
                  fill="url(#econRevGrad)" dot={false}/>
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </SectionCard>
...
function QueueDashboard({ queue }: { queue: QueueData | null }) {
  ...
  const healthPct = queue.queuedCount > 0 ? Math.max(0, 100 - Math.min(100, queue.queuedCount * 5)) : 100
    ...
    <div className="space-y-5">
      ...
      <div className="bg-white border border-zinc-100 rounded-2xl p-5" style={{boxShadow:"0 1px 3px rgba(0,0,0,0.04)"}}>
        ...
        <div className="h-3 bg-zinc-100 rounded-full overflow-hidden">
          <motion.div initial={{width:0}} animate={{width:`${healthPct}%`}} transition={{duration:0.8,ease:"easeOut"}}
            className={cn("h-full rounded-full", healthPct>=80?"bg-emerald-400":healthPct>=50?"bg-amber-400":"bg-red-400")}/>
        </div>
        <div className="flex justify-between mt-2 text-[10px] text-zinc-400"><span>Overloaded</span><span>Healthy</span></div>
      </div>
            ...
            {queue.queuedJobs.length===0
              ...
              : queue.queuedJobs.map(j => (
                <div key={j.id} className="px-5 py-2.5">
                  ...
                  <p className="text-[10px] text-zinc-400 mt-0.5"><code className="font-mono">{j.id.slice(0,8)}</code> · waiting {Math.round(j.waitMs/1000)}s</p>
                </div>
              ))}
          </div>
        </SectionCard>
            ...
            : <div className="flex items-center gap-4 px-5 py-4">
                <ResponsiveContainer width={160} height={160}>
                  <PieChart>
                    ...
                    <Tooltip formatter={(v:any)=>[formatNumber(v),"executions"]} contentStyle={{borderRadius:10,border:"1px solid #f4f4f5",fontSize:11}}/>
                  </PieChart>
                </ResponsiveContainer>
                ...
                <div key={a.id} className="flex items-center gap-3 px-5 py-2.5">
                  ...
                  <div className="flex-1 min-w-0">
                    ...
                    <div className="h-1 bg-zinc-100 rounded-full mt-1.5 overflow-hidden">
                      <div className="h-full rounded-full" style={{width:`${Math.min(100,(rawVal/maxVal)*100)}%`,background:view==="failures"?CHART_COLORS.red:view==="rating"?CHART_COLORS.amber:CHART_COLORS.primary}}/>
                    </div>
                  </div>
                  <p className={cn("text-xs font-bold tabular-nums",view==="failures"?"text-red-600":view==="rating"?"text-amber-600":"text-zinc-900")}>{displayVal}</p>
                </div>
              )
            })}
...
function ExecHealthPanel({ health }: { health: ExecHealth | null }) {
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="bg-white border border-zinc-100 rounded-2xl p-5" style={{boxShadow:"0 1px 3px rgba(0,0,0,0.04)"}}>
        <div className="flex items-center justify-between mb-3">
          <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full bg-emerald-400 animate-pulse"/><p className="text-sm font-semibold text-zinc-900">Last Hour Snapshot</p></div>
          <span className="text-[10px] text-zinc-400 bg-zinc-50 border border-zinc-100 px-2 py-1 rounded-lg">Live</span>
        </div>
        <div className="grid grid-cols-3 gap-6 mb-4">
          <div className="text-center"><p className="text-2xl font-bold text-zinc-900 tabular-nums">{health.total1h}</p><p className="text-[11px] text-zinc-400 mt-1">executions</p></div>
          <div className="text-center"><p className="text-2xl font-bold text-emerald-600 tabular-nums">{health.success1h}</p><p className="text-[11px] text-zinc-400 mt-1">succeeded</p></div>
          <div className="text-center"><p className="text-2xl font-bold text-red-500 tabular-nums">{health.failed1h}</p><p className="text-[11px] text-zinc-400 mt-1">failed</p></div>
        </div>
        {health.total1h > 0 && (
      ...
      <SectionCard title="Live Execution Feed" sub="last 50" icon={Activity}>
        <div className="grid grid-cols-12 gap-2 px-5 py-2 border-b border-zinc-50 text-[10px] font-semibold text-zinc-400 uppercase tracking-wider">
          <div className="col-span-2">ID</div><div className="col-span-2">Agent</div><div className="col-span-2">Model</div><div className="col-span-2">User</div><div className="col-span-1">ms</div><div className="col-span-1">Cost</div><div className="col-span-1">Status</div><div className="col-span-1">When</div>
        </div>
...
function modelColor(m:string){ const l=m?.toLowerCase()??""; return l.includes("haiku")?"#6366f1":l.includes("sonnet")?"#8b5cf6":l.includes("opus")?"#ec4899":"#94a3b8" }
...
function RoutingPanel({ routing }: { routing: Routing | null }) {
  ...
  const barData = routing.modelBreakdown.map(m=>({ name:m.model.replace("claude-","").split("-")[0], usage:m.pct }))
  return (
    <div className="space-y-5">
            ...
            : <div className="px-4 pt-5 pb-3">
                <ResponsiveContainer width="100%" height={160}>
                  <BarChart data={barData} layout="vertical" margin={{top:0,right:8,bottom:0,left:0}}>
                    ...
                    <Tooltip formatter={(v:any,n:string)=>[`${v}${n==="usage"?"%":""}`]} contentStyle={{borderRadius:10,border:"1px solid #f4f4f5",fontSize:11}}/>
                    <Bar dataKey="usage" radius={[0,4,4,0]}>{barData.map((_,i)=><Cell key={i} fill={PIE_PALETTE[i%PIE_PALETTE.length]}/>)}</Bar>
                  </BarChart>
                </ResponsiveContainer>
              </div>}
        </SectionCard>
            ...
            {routing.modelBreakdown.slice(0,5).map(m=>(
              <div key={m.model} className="px-5 py-3">
                <div className="flex items-center justify-between mb-2">
                  <div className="flex items-center gap-2"><div className="w-2 h-2 rounded-full" style={{background:modelColor(m.model)}}/><p className="text-xs font-semibold text-zinc-900">{m.model.replace("claude-","Claude ")}</p></div>
                  <span className="text-[10px] text-zinc-400">{formatNumber(m.count)} runs · {m.pct}%</span>
                </div>
                <div className="h-1.5 bg-zinc-100 rounded-full overflow-hidden mb-2"><div className="h-full rounded-full" style={{width:`${m.pct}%`,background:modelColor(m.model)}}/></div>
                <div className="flex gap-4 text-[10px] text-zinc-400">
                  <span>Avg ${m.avgCost.toFixed(5)}</span><span>{m.avgLatency}ms</span>
            ...
            {routing.routingReasons.slice(0,8).map(r=>{
              ...
              return (
                <div key={r.reason} className="flex items-center gap-3">
                  ...
                  <div className="flex-1 h-1.5 bg-zinc-100 rounded-full overflow-hidden"><div className="h-full bg-violet-400 rounded-full" style={{width:`${(r.count/max)*100}%`}}/></div>
  ...
  return (
    <div className="space-y-5">
      ...
      <SectionCard title="Injection Attempts" sub="all logged attempts" icon={Shield}>
        <div className="grid grid-cols-12 gap-2 px-5 py-2 border-b border-zinc-50 text-[10px] font-semibold text-zinc-400 uppercase tracking-wider">
          <div className="col-span-1">Action</div><div className="col-span-2">Pattern</div><div className="col-span-1">Score</div><div className="col-span-5">Input preview</div><div className="col-span-2">User</div><div className="col-span-1">Time</div>
        </div>
        <div className="max-h-80 overflow-y-auto divide-y divide-zinc-50">
          {flaggedAttempts.length===0
            ? <div className="text-center py-12 flex flex-col items-center gap-2"><Shield className="h-7 w-7 text-emerald-400"/><p className="text-sm text-zinc-400">No injection attempts — platform is clean ✓</p></div>
      ...
      <div className="px-5 py-4">
        <div className="flex items-start gap-4">
          ...
          <div className="flex items-center gap-2 flex-shrink-0">
            <button onClick={()=>setExpanded(v=>!v)} className="flex items-center gap-1 text-xs text-zinc-500 hover:text-zinc-900 px-2.5 py-1.5 rounded-lg hover:bg-zinc-50 transition-colors">
              {expanded?<><ChevronUp className="h-3.5 w-3.5"/>Less</>:<><ChevronDown className="h-3.5 w-3.5"/>Details</>}
            </button>
            <button onClick={doApprove} disabled={loading!==null} className="flex items-center gap-1.5 text-xs font-semibold text-emerald-600 bg-emerald-50 border border-emerald-100 hover:bg-emerald-100 px-3 py-1.5 rounded-lg transition-colors disabled:opacity-50">
              <CheckCircle className="h-3.5 w-3.5"/>{loading==="approve"?"Approving…":"Approve"}
            </button>
            <button onClick={()=>setRejecting(v=>!v)} className="flex items-center gap-1.5 text-xs font-semibold text-red-600 bg-red-50 border border-red-100 hover:bg-red-100 px-3 py-1.5 rounded-lg transition-colors">
        ...
        <AnimatePresence>
          {rejecting && (
            <motion.div initial={{height:0,opacity:0}} animate={{height:"auto",opacity:1}} exit={{height:0,opacity:0}} className="mt-3 space-y-2 overflow-hidden">
              <Textarea value={rejectReason} onChange={e=>setRejectReason(e.target.value)} rows={2} placeholder="Rejection reason (shown to seller)…" className="rounded-xl border-zinc-200 text-sm resize-none"/>
              <div className="flex gap-2">
                <button onClick={doReject} disabled={loading!==null} className="text-xs font-semibold text-white bg-red-500 hover:bg-red-600 px-3 py-1.5 rounded-lg disabled:opacity-50">{loading==="reject"?"Rejecting…":"Confirm Reject"}</button>
          ...
          <motion.div initial={{height:0,opacity:0}} animate={{height:"auto",opacity:1}} exit={{height:0,opacity:0}}
            ...
            <div className="grid grid-cols-3 gap-3">
              {[{l:"Model",v:agent.model_name?.replace("claude-","Claude ")},{l:"Pricing",v:agent.pricing_model?.replace(/_/g," ")},{l:"Max tokens",v:String(agent.max_tokens)}].map(f=>(
                <div key={f.l}><p className="text-[10px] text-zinc-400 font-semibold uppercase tracking-wider">{f.l}</p><p className="text-xs font-semibold text-zinc-700 capitalize">{f.v}</p></div>
              ))}
            </div>
            ...
            {agent.profiles && (
              <div className="flex items-center gap-3 bg-white border border-zinc-100 rounded-xl px-4 py-3">
                ...
                <div><p className="text-xs font-semibold text-zinc-900">{agent.profiles.full_name||"Unknown seller"}</p><p className="text-[11px] text-zinc-400">{agent.profiles.email}</p></div>
...
export function AdminClient({
  ...
  const rejectReview = useCallback(async(id:string,reason:string)=>{
    ...
    const j=await r.json(); if(!r.ok){toast.error(j.error||"Failed to reject");return}
    setPending(p=>p.filter(a=>a.id!==id)); setAgents(a=>a.map(x=>x.id===id?{...x,status:"draft"}:x))
    toast.success("Agent rejected — seller notified")
  },[])

  const approveAgent = async(id:string)=>{
    const r=await fetch("/api/admin/agents",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({agent_id:id,action:"approve"})})
    const j=await r.json(); if(!r.ok){toast.error(j.error);return}
    setAgents(a=>a.map(x=>x.id===id?{...x,status:"active"}:x)); toast.success("Agent approved ✓")
  }
  const suspendAgent = async(id:string)=>{
    ...
    const j=await r.json(); if(!r.ok){toast.error(j.error);return}
    setAgents(a=>a.map(x=>x.id===id?{...x,status:"suspended"}:x)); toast.success("Agent suspended")
  }
  ...
  const filteredAgents = agents.filter(a=>(!agentSearch||a.name?.toLowerCase().includes(agentSearch.toLowerCase()))&&(agentFilter==="all"||a.status===agentFilter))
  ...
  const [activeTab, setActiveTab] = useState(pendingCount > 0 ? "reviews" : "cockpit")
    ...
    <div className="flex min-h-screen bg-zinc-50">
      ...
      <main className="flex-1 overflow-auto">
        ...
        <div className="sticky top-0 z-20 bg-white border-b border-zinc-100" style={{boxShadow:"0 1px 0 rgba(0,0,0,0.04)"}}>
          <div className="max-w-screen-2xl mx-auto px-6 py-4 flex items-center justify-between">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-[11px] text-zinc-400 leading-none mt-0.5">Platform operations · Economics · Runtime</p>
              </div>
            </div>
            <div className="flex items-center gap-3">
              {pendingCount > 0 && (
                <button onClick={()=>setActiveTab("reviews")} className="flex items-center gap-2 bg-amber-50 border border-amber-100 rounded-xl px-3 py-1.5 text-sm font-semibold text-amber-700 hover:bg-amber-100 transition-colors">
                  <AlertCircle className="h-3.5 w-3.5"/>{pendingCount} pending review
                </button>
              )}
              {flaggedCount > 0 && (
                <button onClick={()=>setActiveTab("security")} className="flex items-center gap-2 bg-red-50 border border-red-100 rounded-xl px-3 py-1.5 text-sm font-semibold text-red-600 hover:bg-red-100 transition-colors">
                  <Shield className="h-3.5 w-3.5"/>{flaggedCount} flags
                </button>
              )}
          ...
          {/* Tab rail */}
          <div className="max-w-screen-2xl mx-auto px-6 pb-0 overflow-x-auto scrollbar-none">
            <div className="flex">
              {tabs.map(t=>(
                <button key={t.id} onClick={()=>setActiveTab(t.id)} className={cn(
                  "flex items-center gap-1.5 px-3.5 py-2.5 text-xs font-medium whitespace-nowrap border-b-2 transition-all",
                  activeTab===t.id ? "border-zinc-900 text-zinc-900"
                    : (t as any).danger ? "border-transparent text-red-500 hover:text-red-600"
                ...
                <div className="space-y-4">
                  {pendingCount===0
                    ? <div className="bg-white border border-zinc-100 rounded-2xl flex flex-col items-center justify-center py-16 text-center" style={{boxShadow:"0 1px 3px rgba(0,0,0,0.04)"}}>
                        ...
                        <p className="text-xs text-zinc-400">No submissions waiting for review.</p>
                      </div>
                    : <>
                        ...
                        <div className="space-y-3">{pending.map(a=><ReviewCard key={a.id} agent={a} onApprove={approveReview} onReject={rejectReview}/>)}</div>
                      </>}
                </div>
              )}
                      ...
                      {filteredAgents.length===0
                        ...
                        : filteredAgents.map(a=>(
                          <div key={a.id} className="grid grid-cols-12 gap-3 px-5 py-3.5 items-center hover:bg-zinc-50/50 transition-colors">
                            <div className="col-span-4 min-w-0"><p className="font-medium text-sm text-zinc-900 truncate">{a.name}</p><p className="text-xs text-zinc-400 truncate mt-0.5">{a.description}</p></div>
                            <div className="col-span-2"><span className="text-xs text-zinc-500 capitalize">{a.category?.replace(/_/g," ")}</span></div>
                            <div className="col-span-2 min-w-0"><p className="text-xs text-zinc-600 truncate">{a.profiles?.full_name||"—"}</p><p className="text-[11px] text-zinc-400 truncate">{a.profiles?.email||""}</p></div>
                            ...
                            <div className="col-span-2 flex items-center justify-end gap-1.5">
                              <Link href={`/marketplace/${a.id}`} target="_blank"><button className="p-1.5 rounded-lg text-zinc-400 hover:text-zinc-700 hover:bg-zinc-100"><Eye className="h-3.5 w-3.5"/></button></Link>
                              {a.status==="pending_review" && <>
                                <button onClick={()=>approveAgent(a.id)} className="text-xs font-semibold text-emerald-600 bg-emerald-50 border border-emerald-100 hover:bg-emerald-100 px-2.5 py-1 rounded-lg flex items-center gap-1"><CheckCircle className="h-3 w-3"/>Approve</button>
                                <button onClick={()=>suspendAgent(a.id)} className="text-xs font-semibold text-red-600 bg-red-50 border border-red-100 hover:bg-red-100 px-2.5 py-1 rounded-lg flex items-center gap-1"><XCircle className="h-3 w-3"/>Reject</button>
                              </>}
                      ...
                      {filteredUsers.length===0
                        ...
                        : filteredUsers.map(u=>(
                          <div key={u.id} className={cn("grid grid-cols-12 gap-3 px-5 py-3.5 items-center hover:bg-zinc-50/50 transition-colors",u.is_banned&&"opacity-50")}>
                            ...
                            <div className="col-span-2"><span className={cn("text-[10px] font-bold px-2 py-0.5 rounded-full",u.role==="admin"?"bg-red-50 text-red-600":"bg-zinc-100 text-zinc-500")}>{u.role}</span>{u.is_banned&&<span className="ml-1 text-[10px] font-bold px-2 py-0.5 rounded-full bg-red-50 text-red-600">banned</span>}</div>
                            <div className="col-span-2"><p className="text-xs text-zinc-600 tabular-nums">{formatCurrency(u.total_earned||0)}</p><p className="text-[11px] text-zinc-400">earned</p></div>
                            <div className="col-span-1"><p className="text-xs text-zinc-400">{formatRelativeTime(u.created_at)}</p></div>
                            <div className="col-span-1 flex justify-end">
                              <button onClick={()=>banUser(u.id,u.is_banned)} className={cn("p-1.5 rounded-lg transition-colors",u.is_banned?"text-emerald-500 hover:bg-emerald-50":"text-zinc-400 hover:text-red-500 hover:bg-red-50")} title={u.is_banned?"Unban":"Ban"}>
        ...
        <div className="border-t border-zinc-100 px-6 py-4 max-w-screen-2xl mx-auto">
          <details>
            <summary className="text-[11px] text-zinc-400 cursor-pointer hover:text-zinc-600 flex items-center gap-1.5">
              <ShieldCheck className="h-3 w-3"/>Admin setup · how to grant access
            </summary>
