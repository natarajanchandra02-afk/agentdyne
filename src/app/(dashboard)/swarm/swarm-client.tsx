...
/**
 * AgentDyne — Multi-Agent Swarm v8 (Build-Fixed)
 *
 * FIXED: JSX syntax error on {{ }} braces in Variables panel.
 * Root cause: {"{{"}  and {"}}"} had mismatched closing braces
 * in the prior build (v6). Now rendered as HTML entities &#123;&#123;
 * and &#125;&#125; which are 100% valid JSX and never confuse the parser.
 *
 * All other functionality identical to v7.
 */
...
interface SwarmResult {
  ...
  messageLog: unknown[]; rounds: number
}
...
const MODES = [
  {
    ...
    active: "border-[#c7d2fe] bg-[#eef2ff]",
    check: "#6366f1",
  },
  {
  ...
  },
  {
    ...
    active: "border-[#fde68a] bg-[#fffbeb]",
    check: "#d97706",
  },
]
        ...
        <div className="flex-1 overflow-auto px-3 pb-3">
          <svg width="100%" viewBox={`0 0 ${W} ${SVG_H}`} style={{ minHeight: SVG_H, display: "block" }}>
            ...
            {mids.map((a, i) => {
              ...
              return (
                <g key={a.id}>
                  <rect x={sx} y={WY} width={NW} height={NH+14} rx="10" fill={m.bg} stroke={m.border} strokeWidth="1.5"/>
                  <text x={wxs[i]} y={WY+18} textAnchor="middle" fontSize="11" fontWeight="700" fill={m.color}>{a.name.slice(0,16)}</text>
                  {sub.map((ln, li) => (
                    <text key={li} x={wxs[i]} y={WY+30+li*11} textAnchor="middle" fontSize="9" fill="#a1a1aa">{ln}</text>
                  ))}
                  <text x={wxs[i]} y={WY+NH+8} textAnchor="middle" fontSize="9" fontWeight="600" fill={m.color}>Conf: {88+i*2}%</text>
                </g>
              )
            })}
            {synth && (() => {
              ...
              return (
                <g>
                  <rect x={(W-NW)/2} y={SY} width={NW} height={NH+14} rx="10" fill={sm.bg} stroke={sm.border} strokeWidth="1.5"/>
                  <text x={W/2} y={SY+18} textAnchor="middle" fontSize="11" fontWeight="700" fill={sm.color}>{synth.name.slice(0,16)}</text>
                  {sub.map((ln, li) => (
            ...
            <g>
              ...
              <text x={W/2} y={(synth?OY:WY+NH+24)+20} textAnchor="middle" fontSize="12" fontWeight="700" fill="#15803d">Executive Report</text>
              <text x={W/2} y={(synth?OY:WY+NH+24)+35} textAnchor="middle" fontSize="10" fill="#86efac">Final Output</text>
            </g>
          </svg>
        </div>
        <div className="flex items-center gap-1 px-4 pb-3 flex-shrink-0">
          {[Maximize2, Plus, Minus].map((Icon, i) => (
            <button key={i} type="button"
              className="w-[26px] h-[26px] rounded-lg flex items-center justify-center hover:bg-zinc-100 transition-colors"
    ...
    <div className="flex-1 flex flex-col overflow-hidden">
      ...
      <div className="flex-1 flex items-center justify-center px-4">
        <svg width="100%" viewBox={`0 0 ${W} 210`}>
          <DEFS/>
          <rect x={(W-110)/2} y={8} width={110} height={34} rx="8" fill="#eef2ff" stroke="#c7d2fe" strokeWidth="1.5"/>
...
function LiveExecution({ agents, running, result }: {
  ...
  return (
    <div className="flex-shrink-0" style={{ borderTop: "1px solid #f4f4f5", background: "var(--color-background-primary)" }}>
      <div className="flex items-center gap-2.5 px-5 py-2" style={{ borderBottom: "1px solid #f4f4f5" }}>
        ...
        <div className="flex ml-2">
          {[0, 15, 30, 45, 60].map(t => (
            <span key={t} style={{ width: 52, textAlign: "center", fontSize: 9, color: "#d4d4d8", flexShrink: 0 }}>{t}s</span>
          ))}
        </div>
      </div>
      {steps.map(s => (
        <div key={s.num} className="flex items-center gap-2.5 px-5 py-[6px]"
          ...
          <span className="w-28 truncate flex-shrink-0" style={{ fontSize: 11, fontWeight: 700, color: "var(--color-text-primary)" }}>{s.name}</span>
          <span className="flex-1 truncate min-w-0" style={{ fontSize: 10, color: "#71717a" }}>{s.action}</span>
          <span className={cn("flex-shrink-0 text-[10px] font-bold px-2 py-0.5 rounded-full", PILL[s.status])}>{LABEL[s.status]}</span>
          <span className="w-10 text-right flex-shrink-0 tabular-nums" style={{ fontSize: 10, color: "#a1a1aa" }}>{s.dur}</span>
          <div className="w-[104px] h-5 rounded flex-shrink-0 relative overflow-hidden" style={{ background: "#f4f4f5" }}>
            {s.pct > 0 && (
...
function RightPanel({ agents, mode, rounds, running, result, sessions, templates }: {
  ...
  sessions: unknown[]; templates: unknown[]
}) {
  const m   = calcMetrics(agents, mode, rounds)
  const has = agents.length > 0
  const list = (sessions.length ? sessions : DEMO_SESSIONS) as Array<{ name?: string; id?: string; status: string; date?: string; created_at?: string }>
  const tmpl = templates.length
    ? (templates as Array<{ name: string }>).map(t => t.name)
    : DEMO_TEMPLATES

  return (
    <aside className="flex-shrink-0 overflow-y-auto bg-white" style={{ width: 272, borderLeft: "1px solid #f4f4f5" }}>

      {/* Swarm Intelligence */}
      <div className="px-4 pt-4 pb-3" style={{ borderBottom: "1px solid #f4f4f5" }}>
        <div className="flex items-center gap-2 mb-3">
          <Brain style={{ width: 14, height: 14, color: "#6366f1" }}/>
          <span style={{ fontSize: 13, fontWeight: 700, color: "var(--color-text-primary)" }}>Swarm Intelligence</span>
        </div>
        <div className="grid grid-cols-2 gap-2 mb-3">
          {[
            ...
            <div key={i} className="rounded-xl p-2.5" style={{ background: "#fafafa", border: "1px solid #f4f4f5" }}>
              ...
              <p style={{ fontSize: 20, fontWeight: 700, lineHeight: 1, marginBottom: 4,
                ...
                color: (kpi as { vc?: string }).vc ?? "var(--color-text-primary)" }}>
                {kpi.v}
              </p>
              {kpi.spark && has && <Sparkline color={(kpi as { sc: string }).sc} up={(kpi as { up: boolean }).up}/>}
            </div>
          ))}
        </div>
        {[
          ...
          <div key={label} className="flex items-center gap-2 py-[5px]" style={{ borderBottom: "1px solid rgba(244,244,245,0.9)" }}>
            ...
            <span className="ml-auto text-right truncate" style={{ fontSize: 10, fontWeight: 600, color: "var(--color-text-primary)", maxWidth: 120 }}>{value}</span>
          </div>
        ))}
      </div>
      ...
      <AnimatePresence>
        {result && (
          <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: "auto" }}
            exit={{ opacity: 0, height: 0 }} className="overflow-hidden" style={{ borderBottom: "1px solid #f4f4f5" }}>
            <div className="px-4 py-3">
              <p style={{ fontSize: 12, fontWeight: 700, color: "var(--color-text-primary)", marginBottom: 10 }}>Post Execution Insights</p>
              <div className="flex gap-3">
                ...
                <div className="flex-1 min-w-0">
                  <p style={{ fontSize: 9, fontWeight: 700, color: "#a1a1aa", textTransform: "uppercase", letterSpacing: "0.06em", marginBottom: 4 }}>Key Strengths</p>
                  ...
                  <p style={{ fontSize: 9, fontWeight: 700, color: "#a1a1aa", textTransform: "uppercase", letterSpacing: "0.06em", marginTop: 6, marginBottom: 4 }}>Suggested Improvements</p>
              ...
              <button type="button"
                className="mt-3 w-full flex items-center justify-center gap-1.5 rounded-xl font-bold transition-colors hover:opacity-90"
      ...
      <div className="pt-3 pb-2" style={{ borderBottom: "1px solid #f4f4f5" }}>
        <div className="flex items-center justify-between px-4 mb-2">
          ...
          <button style={{ fontSize: 10, color: "#6366f1", fontWeight: 600 }} className="hover:underline">View all</button>
        </div>
          ...
          return (
            <button key={i} type="button"
              ...
              <div className="w-[22px] h-[22px] rounded-full flex items-center justify-center flex-shrink-0"
                ...
                {ok ? <CheckCircle2 style={{ width: 12, height: 12, color: "#22c55e" }}/> : <AlertCircle style={{ width: 12, height: 12, color: "#dc2626" }}/>}
              </div>
              <div className="flex-1 min-w-0">
                <p className="truncate" style={{ fontSize: 11, fontWeight: 600, color: "var(--color-text-primary)" }}>
              ...
              <ChevronRight style={{ width: 11, height: 11, color: "#d4d4d8", flexShrink: 0, opacity: 0 }}
...
function AgentRow({ agent, idx, selected, onToggle }: {
  ...
  return (
    <button type="button" onClick={onToggle}
      ...
      <GripVertical style={{ width: 13, height: 13, color: "#d4d4d8", flexShrink: 0, opacity: 0 }}
        className="group-hover:opacity-100 transition-opacity"/>
      ...
      <div className="flex-1 min-w-0">
        <p className="truncate" style={{ fontSize: 11, fontWeight: 700, color: "var(--color-text-primary)", lineHeight: 1.3 }}>{agent.name}</p>
        <p style={{ fontSize: 9.5, lineHeight: 1, marginTop: 2, color: meta.color, fontWeight: 600 }}>
          Role: {meta.role}<span style={{ color: "#a1a1aa", fontWeight: 400 }}> · {shortModel(agent.model_name)}</span>
        </p>
      </div>
      <div className="text-right flex-shrink-0">
        <p style={{ fontSize: 11, fontWeight: 700, color: "var(--color-text-primary)", fontVariantNumeric: "tabular-nums" }}>{conf}%</p>
        <p style={{ fontSize: 9.5, color: "#f87171", fontVariantNumeric: "tabular-nums" }}>{cost.toFixed(3)}</p>
      </div>
      <MoreHorizontal style={{ width: 13, height: 13, color: "#a1a1aa", flexShrink: 0, opacity: 0 }}
        className="group-hover:opacity-100 transition-opacity"/>
    </button>
  )
}
...
export default function SwarmClient() {
  ...
  const handleFileChange = useCallback((e: ChangeEvent<HTMLInputElement>) => {
    ...
    if (tooLarge.length) { toast.error(`${tooLarge.map(f => f.name).join(", ")} exceed 10 MB`); e.target.value = ""; return }
  ...
  const runSwarm = useCallback(async () => {
    ...
    try {
      ...
      const msg = err instanceof Error ? err.message : "Swarm execution failed"
  ...
  return (
    <div className="-mx-6 -my-8 flex flex-col"
      style={{ height: "calc(100vh)", minHeight: 720, overflow: "hidden", background: "var(--color-background-primary)" }}>

      {/* TOP BAR */}
      <header className="flex items-center gap-3 flex-shrink-0"
        style={{ padding: "10px 20px", borderBottom: "1px solid #f4f4f5", minHeight: 52,
          background: "var(--color-background-primary)", boxShadow: "0 1px 0 rgba(0,0,0,0.03)" }}>
        <div>
          <h1 style={{ fontSize: 17, fontWeight: 700, letterSpacing: -0.4, color: "var(--color-text-primary)", lineHeight: 1.2 }}>
            Multi-Agent Swarm
          </h1>
          <p style={{ fontSize: 11, color: "#a1a1aa", marginTop: 1 }}>Build, visualize, and execute intelligent agent teams.</p>
        </div>
        <div className="ml-auto flex items-center gap-2">
          {[{ Icon: LayoutTemplate, label: "Templates" }, { Icon: Save, label: "Save as Template" }].map(({ Icon, label }) => (
            <button key={label} type="button"
              className="flex items-center gap-1.5 rounded-[10px] font-semibold transition-all hover:bg-zinc-50 active:scale-95"
              style={{ height: 32, padding: "0 12px", border: "1px solid #e4e4e7", fontSize: 11,
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="flex-shrink-0 flex flex-col overflow-y-auto"
          style={{ width: 268, borderRight: "1px solid #f4f4f5", background: "var(--color-background-primary)" }}>
            ...
            <div className="grid grid-cols-3 gap-1.5">
              {MODES.map(m => (
                <button key={m.id} type="button" onClick={() => setMode(m.id)}
                  className={cn(
                    "relative flex flex-col items-center text-center transition-all rounded-xl",
                    "focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400 active:scale-95",
          ...
          <div style={{ padding: "12px", borderBottom: "1px solid #f4f4f5" }}>
            <p style={{ fontSize: 10, fontWeight: 700, textTransform: "uppercase", letterSpacing: "0.06em", color: "#71717a", marginBottom: 6 }}>Task</p>
            <textarea value={task} onChange={e => setTask(e.target.value)} rows={4}
              ...
              style={{ fontSize: 12, lineHeight: 1.55, padding: "8px 10px", border: "1px solid #e4e4e7",
                color: "var(--color-text-primary)", background: "var(--color-background-primary)", fontFamily: "inherit" }}
              ...
              <div className="flex flex-wrap gap-1.5 mt-2">
                {files.map(f => (
                  <div key={f.id} className="flex items-center gap-1 rounded-lg pl-1.5 pr-1 py-1 group"
                    ...
                    <span className="truncate" style={{ fontSize: 9.5, color: "var(--color-text-secondary)", maxWidth: 72 }}>{f.file.name}</span>
                    <button type="button" onClick={() => removeFile(f.id)}
                      className="flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity ml-0.5 p-0.5 rounded hover:bg-zinc-200">
            ...
            <AnimatePresence>
              {showCtx && (
                <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }}
                  ...
                  <div className="px-3 py-2 flex items-center gap-2" style={{ borderBottom: "1px solid #f4f4f5" }}>
                    <AlignLeft style={{ width: 11, height: 11, color: "#3b82f6" }}/>
                    <span style={{ fontSize: 11, fontWeight: 700, color: "var(--color-text-primary)" }}>Additional Context</span>
                    <span style={{ fontSize: 9.5, color: "#a1a1aa" }}>Appended to every agent prompt</span>
                  </div>
                  <textarea value={context} onChange={e => setContext(e.target.value)} rows={3}
                    placeholder="Paste any background info, constraints, or data the agents should know…"
                    className="w-full focus:outline-none resize-none"
                    style={{ fontSize: 11, color: "var(--color-text-primary)", lineHeight: 1.5,
                      background: "transparent", fontFamily: "inherit", padding: "8px 12px" }}/>
                </motion.div>
              )}
            </AnimatePresence>

            {/* Variables panel — FIXED: uses HTML entities for {{ }} to avoid JSX parser confusion */}
            <AnimatePresence>
              {showVars && (
                <motion.div initial={{ opacity: 0, y: -4 }} animate={{ opacity: 1, y: 0 }}
                  exit={{ opacity: 0, y: -4 }} transition={{ duration: 0.12 }}
                  ...
                  <div className="px-3 py-2 flex items-center gap-2" style={{ borderBottom: "1px solid #f4f4f5" }}>
                    ...
                    <span style={{ fontSize: 11, fontWeight: 700, color: "var(--color-text-primary)" }}>Variables</span>
                  ...
                  <div className="p-2 space-y-1.5">
                      ...
                      <div key={v.id} className="flex items-center gap-1.5">
                        <div className="flex items-center rounded-lg overflow-hidden flex-1"
                          ...
                          <div className="flex items-center gap-0.5 px-2 py-1"
                            ...
                            {/* ✅ HTML entities — safe in all JSX parsers, render as {{ }} in browser */}
                            ...
                            <input value={v.key} onChange={e => updVar(v.id, "key", e.target.value)}
                              ...
                              style={{ fontSize: 11, width: 56, color: "#6366f1", fontWeight: 600, fontFamily: "monospace" }}/>
                            <span style={{ fontSize: 10, color: "#a1a1aa", fontFamily: "monospace" }}>&#125;&#125;</span>
                          </div>
                          <input value={v.value} onChange={e => updVar(v.id, "value", e.target.value)}
                            ...
                            style={{ fontSize: 11, color: "var(--color-text-primary)", background: "var(--color-background-primary)" }}/>
                        </div>
                    ...
                    {vars.length > 0 && (
                      <button type="button" onClick={applyVars}
                        className="w-full flex items-center justify-center gap-1.5 rounded-lg py-1.5 transition-colors hover:opacity-90"
                        style={{ background: "rgba(99,102,241,0.08)", fontSize: 10.5, color: "#6366f1", fontWeight: 600 }}>
            ...
            {[
              ...
              <div key={s.label} style={{ marginBottom: 12 }}>
                <div className="flex items-center justify-between" style={{ marginBottom: 4 }}>
                  ...
                  <span style={{ fontSize: 10, fontWeight: 700, color: "var(--color-text-primary)", fontVariantNumeric: "tabular-nums" }}>{s.display}</span>
                </div>
                <input type="range" min={s.min} max={s.max} step={s.step} value={s.val}
                  onChange={e => s.set(parseFloat(e.target.value))}
                  className="w-full cursor-pointer" style={{ height: 4, accentColor: "#6366f1", display: "block" }}/>
          ...
          <div style={{ padding: "12px", borderBottom: "1px solid #f4f4f5" }}>
            {[
              ...
              <div key={f.label} className="flex items-center gap-2.5" style={{ marginBottom: 10 }}>
                ...
                <div className="flex-1 min-w-0">
                  <p style={{ fontSize: 11, fontWeight: 700, color: "var(--color-text-primary)", lineHeight: 1.2 }}>{f.label}</p>
                ...
                {f.action && (
                  <button type="button" onClick={f.action} disabled={f.loading}
                    ...
                    style={{ width: 22, height: 22, background: "#f4f4f5", border: "none", flexShrink: 0, cursor: "pointer" }}>
                    {f.loading
          ...
          <div className="flex-1 flex flex-col min-h-0">
            <div className="flex items-center justify-between px-3 py-2" style={{ borderBottom: "1px solid #f4f4f5" }}>
              <p style={{ fontSize: 11, fontWeight: 700, color: "var(--color-text-primary)" }}>
                Selected Agents <span style={{ fontWeight: 400, color: "#a1a1aa" }}>({selAgents.length})</span>
              </p>
              {selAgents.length > 0 && (
                <button type="button" onClick={() => setSelected([])}
                  style={{ fontSize: 10, color: "#a1a1aa", fontWeight: 500 }} className="hover:text-zinc-600 transition-colors">
                  Clear all
                </button>
              )}
            </div>
            {agentsLoading ? (
              <div className="flex items-center gap-2 px-4 py-6" style={{ color: "#a1a1aa", fontSize: 12 }}>
              ...
              <div className="px-4 py-8 text-center space-y-2">
                ...
                <a href="/builder" style={{ fontSize: 12, color: "#6366f1" }} className="underline font-semibold">Create your first agent →</a>
              </div>
            ) : (
        ...
        {/* COL B — CENTRE */}
        <main className="flex-1 flex flex-col overflow-hidden min-w-0" style={{ background: "var(--color-background-primary)" }}>
          ...
          <div className="flex flex-1 overflow-hidden">
            <div className="flex-1 flex flex-col overflow-hidden">
              <div className="flex items-center gap-2 px-4 py-2.5 flex-shrink-0" style={{ borderBottom: "1px solid #f4f4f5" }}>
                <GitBranch style={{ width: 13, height: 13, color: "#a1a1aa" }}/>
                <span style={{ fontSize: 11, fontWeight: 700, color: "var(--color-text-primary)" }}>Swarm Graph</span>
                {running && (
                  <span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold animate-pulse"
                    style={{ background: "#eff6ff", color: "#3b82f6", border: "1px solid #bfdbfe" }}>● Live</span>
                )}
                {result && (
                  <span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold"
                    style={{ background: "#f0fdf4", color: "#16a34a", border: "1px solid #bbf7d0" }}>✓ Complete</span>
                )}
              </div>
            ...
            {mode === "debate" && (
              <aside className="flex-shrink-0 overflow-y-auto" style={{ width: 200, borderLeft: "1px solid #f4f4f5", padding: 14 }}>
                <p style={{ fontSize: 12, fontWeight: 700, color: "var(--color-text-primary)", marginBottom: 14 }}>Debate Settings</p>
                <div style={{ marginBottom: 14 }}>
                  ...
                  <div className="flex items-center gap-2">
                    <input type="number" min={1} max={10} value={rounds}
                      ...
                      style={{ width: 52, height: 28, fontSize: 13, borderRadius: 7, border: "1px solid #e4e4e7",
                ...
                <div style={{ marginBottom: 14 }}>
                  <p style={{ fontSize: 10, fontWeight: 600, color: "#71717a", marginBottom: 7 }}>Consensus Method</p>
                    ...
                    {CONSENSUS.map(method => (
                      <label key={method} className="flex items-center gap-2 cursor-pointer" onClick={() => setCm(method)}>
                        <div className="w-3.5 h-3.5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all"
                          style={{ borderColor: cm === method ? "#3b82f6" : "#d4d4d8",
                ...
                <div style={{ marginBottom: 14 }}>
                  ...
                  <div className="relative">
                    <select value={arbiter} onChange={e => setArbiter(e.target.value)}
                      ...
                      style={{ height: 28, fontSize: 10.5, borderRadius: 7, border: "1px solid #e4e4e7",
                        paddingLeft: 8, paddingRight: 22, color: "var(--color-text-primary)",
                    ...
                    <ChevronDown style={{ width: 11, height: 11, color: "#a1a1aa", position: "absolute", right: 7, top: "50%", transform: "translateY(-50%)", pointerEvents: "none" }}/>
                  </div>
                </div>
                <div style={{ marginBottom: 14 }}>
                  <p style={{ fontSize: 10, fontWeight: 600, color: "#71717a", marginBottom: 5 }}>Conflict Resolution</p>
                  <div className="relative">
                ...
                <div>
                  <p style={{ fontSize: 10, fontWeight: 600, color: "#71717a", marginBottom: 5 }}>Early Stopping</p>
                  <div className="flex items-center justify-between">
                    <span style={{ fontSize: 9.5, color: "#a1a1aa", flex: 1, paddingRight: 8 }}>Stop when consensus reached</span>
          ...
          <AnimatePresence>
            {result?.finalAnswer && (
              <motion.div initial={{ opacity: 0, y: 6 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                ...
                style={{ borderColor: "#f4f4f5", padding: "14px 18px", background: "var(--color-background-primary)" }}>
                <div className="flex items-center gap-2" style={{ marginBottom: 8 }}>
                  <CheckCircle2 style={{ width: 14, height: 14, color: "#22c55e" }}/>
                  <p style={{ fontSize: 12, fontWeight: 700, color: "var(--color-text-primary)", flex: 1 }}>Final Answer</p>
                  <button type="button"
                    onClick={() => { navigator.clipboard.writeText(result.finalAnswer); toast.success("Copied to clipboard") }}
                ...
                <div className="overflow-y-auto rounded-xl"
                  style={{ maxHeight: 160, background: "#fafafa", padding: 12, fontSize: 12, lineHeight: 1.65,
