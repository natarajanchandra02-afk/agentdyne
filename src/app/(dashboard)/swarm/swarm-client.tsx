...
/**
 * AgentDyne — Multi-Agent Swarm v10 (Build-Fixed)
 * JSX syntax fix: replaced {"{{"}  and {"}}"} with safe string constants
 */
...
import {
  ...
  ArrowRight, Clock, DollarSign, Target, List,
} from "lucide-react"
...
// Safe curly-brace strings — avoids JSX parser ambiguity
...
const Spark = ({color,up=true}:{color:string;up?:boolean}) => {
  ...
  return <svg width="72" height="14" viewBox="0 0 72 14" aria-hidden><path d={d} stroke={color} strokeWidth="1.5" strokeLinecap="round" fill="none"/><path d={`${d} L72,14 L0,14 Z`} fill={color} opacity="0.09"/></svg>
}

const Toggle = ({on,set}:{on:boolean;set:(v:boolean)=>void}) => (
  <button type="button" role="switch" aria-checked={on} onClick={()=>set(!on)}
    className="relative flex-shrink-0 rounded-full transition-colors duration-150 focus:outline-none"
...
const CB = ({on,set}:{on:boolean;set:(v:boolean)=>void}) => (
  <button type="button" role="checkbox" aria-checked={on} onClick={()=>set(!on)}
    className="flex items-center justify-center flex-shrink-0 rounded-[5px] transition-all focus:outline-none"
...
const Modal = ({open,close,title,children,width=500}:{open:boolean;close:()=>void;title:string;children:React.ReactNode;width?:number}) => {
  useEffect(()=>{
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          ...
          <motion.div initial={{opacity:0,scale:0.96,y:8}} animate={{opacity:1,scale:1,y:0}}
            ...
            <div className="flex items-center justify-between px-5 py-3.5 flex-shrink-0" style={{borderBottom:`1px solid ${T.border}`}}>
              ...
              <button onClick={close} className="w-7 h-7 rounded-lg flex items-center justify-center hover:bg-zinc-100"><X style={{width:13,height:13,color:T.text3}}/></button>
            </div>
  ...
  return (
    <Modal open={open} close={close} title="Upgrade to Launch Swarms" width={440}>
      <div className="p-5 space-y-4">
        <div className="w-12 h-12 rounded-2xl flex items-center justify-center mx-auto" style={{background:T.brandBg}}>
        ...
        <div className="text-center">
          ...
          <p style={{fontSize:12.5,color:T.text2,lineHeight:1.65}}>Multi-agent swarms require Starter plan or above. Coordinate up to 8 agents, unlock all modes, and build swarm memory.</p>
        </div>
        <div className="rounded-xl p-4 space-y-2.5" style={{background:T.soft,border:`1px solid ${T.border}`}}>
          {["Up to 8 agents per swarm run","Orchestrate, Debate & Parallel modes","Swarm memory & learnings across runs","Live execution timeline & full trace","Post-execution insights & v2 creation"].map(f=>(
        ...
        <div className="flex gap-2.5">
          <button onClick={close} className="flex-1 h-10 rounded-xl font-semibold hover:bg-zinc-50 transition-colors" style={{border:`1px solid ${T.border2}`,fontSize:13,color:T.text2,background:T.bg}}>Maybe later</button>
          <button onClick={()=>{close();router.push("/billing")}} className="flex-1 h-10 rounded-xl font-bold flex items-center justify-center gap-2" style={{background:T.brand,color:"#fff",fontSize:13,border:"none"}}>
...
const DEMO_TEMPLATES: Template[] = [
  ...
  {id:"t3",name:"Due Diligence Swarm",mode:"parallel",description:"Financial · Legal · Market · Risk — parallel",agent_roles:[]},
...
const TemplatesModal = ({open,close,templates,onLoad}:{open:boolean;close:()=>void;templates:Template[];onLoad:(t:Template)=>void}) => {
  ...
  return (
    <Modal open={open} close={close} title="Swarm Templates" width={540}>
      <div className="p-4 space-y-2">
        <p style={{fontSize:11,color:T.text3,paddingBottom:2}}>Select a template to load its configuration</p>
        {list.map(t=>(
          <button key={t.id} type="button" onClick={()=>{onLoad(t);close()}}
            className="w-full flex items-start gap-3 p-3.5 rounded-xl text-left transition-all hover:bg-zinc-50"
            style={{border:`1px solid ${T.border}`}}>
            <div className="w-9 h-9 rounded-xl flex items-center justify-center flex-shrink-0" style={{background:`${MC[t.mode]??T.brand}14`}}>
            ...
            <div className="flex-1 min-w-0">
              <div className="flex items-center gap-2 mb-0.5">
                ...
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full capitalize" style={{background:`${MC[t.mode]??T.brand}14`,color:MC[t.mode]??T.brand}}>{t.mode}</span>
              </div>
...
const SaveModal = ({open,close,mode,agentCount,onSave}:{open:boolean;close:()=>void;mode:string;agentCount:number;onSave:(n:string,d:string)=>Promise<void>}) => {
    ...
    <Modal open={open} close={close} title="Save as Template" width={420}>
      <div className="p-5 space-y-4">
        <div className="flex items-center gap-3 p-3 rounded-xl" style={{background:T.soft,border:`1px solid ${T.border}`}}>
          <div className="w-8 h-8 rounded-lg flex items-center justify-center flex-shrink-0" style={{background:`${T.brand}14`}}>
        ...
        <div className="flex gap-2.5">
          <button onClick={close} className="flex-1 h-10 rounded-xl font-semibold hover:bg-zinc-50 transition-colors" style={{border:`1px solid ${T.border2}`,fontSize:13,color:T.text2}}>Cancel</button>
          <button onClick={handle} disabled={saving||!name.trim()} className="flex-1 h-10 rounded-xl font-bold transition-all"
            style={{background:name.trim()?T.brand:`${T.brand}60`,color:"#fff",fontSize:13,border:"none",cursor:name.trim()?"pointer":"default"}}>
...
const TraceModal = ({open,close,log}:{open:boolean;close:()=>void;log:any[]}) => (
  <Modal open={open} close={close} title="Execution Trace" width={700}>
    <div className="p-4 space-y-2.5">
      {!log.length&&<div className="text-center py-10"><Eye style={{width:28,height:28,color:T.text4,margin:"0 auto 8px"}}/><p style={{fontSize:12,color:T.text3}}>No trace data yet. Run the swarm first.</p></div>}
      {log.map((entry,i)=>{
        ...
        return (
          <div key={i} className="rounded-xl p-3" style={{background:T.soft,border:`1px solid ${T.border}`}}>
            <div className="flex items-center gap-2 mb-2">
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full" style={{background:isOrch?`${T.brand}14`:`${T.green}14`,color:isOrch?T.brand:T.greenTx}}>{from}</span>
              {entry.timestamp&&<span style={{fontSize:9,color:T.text4}}>{new Date(entry.timestamp).toLocaleTimeString()}</span>}
            </div>
            <pre style={{background:T.bg,color:T.text1,fontSize:10,fontFamily:"monospace",borderRadius:8,padding:"8px 10px",maxHeight:160,overflow:"auto",whiteSpace:"pre-wrap"}}>
...
const PlanCard = ({agents,mode,rounds}:{agents:Agent[];mode:SwarmMode;rounds:number}) => {
  ...
  const steps=mode==="orchestrate"?[
    ...
    ...agents.slice(1,agents.length>3?agents.length-1:agents.length).map(a=>{const r=getRoleMeta(a);return{label:r.role+": "+a.name,desc:r.role==="Research"?"Gather & research":"Execute sub-task"}}),
  ...
  return (
    <motion.div initial={{opacity:0,y:-4}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-4}}
      ...
      <div className="flex items-center justify-between px-4 py-2.5" style={{borderBottom:`1px solid ${T.brandBd}`}}>
        ...
        <div className="flex items-center gap-3.5">
          {[{Icon:Clock,v:`~${m.secs}s`},{Icon:DollarSign,v:`~$${m.cost.toFixed(3)}`},{Icon:Target,v:`${m.acc}% confidence`}].map(({Icon,v})=>(
            <div key={v} className="flex items-center gap-1"><Icon style={{width:10,height:10,color:T.brand}}/><span style={{fontSize:10,fontWeight:600,color:T.brand}}>{v}</span></div>
          ))}
        </div>
      </div>
...
const SwarmGraph = ({agents,mode,running}:{agents:Agent[];mode:SwarmMode;running:boolean}) => {
  ...
  const AD=({id,stroke}:{id:string;stroke:string})=>(
  ...
  if(!agents.length)return(
    <div className="flex-1 flex flex-col items-center justify-center gap-3 select-none py-8">
      ...
      <p style={{fontSize:12,color:T.text3,fontWeight:500}}>Select 2 or more agents to preview the swarm graph</p>
    </div>
  )
  const Node=({x,y,w,h,agent,conf,pulse}:{x:number;y:number;w:number;h:number;agent:Agent;conf?:number;pulse?:boolean})=>{
    ...
    return(
      <g>
        ...
        {pulse&&<rect x={x+4} y={y+h-5} height="3" rx="1.5" fill={m.color} opacity="0.35"><animate attributeName="x" values={`${x+4};${x+w-18};${x+4}`} dur="1.8s" repeatCount="indefinite"/><animate attributeName="width" values="12;26;12" dur="1.8s" repeatCount="indefinite"/></rect>}
      </g>
    )
  }
  if(mode==="orchestrate"&&agents.length>=2){
    const planner=agents[0],workers=agents.slice(1,agents.length>3?agents.length-1:agents.length),synth=agents.length>=3?agents[agents.length-1]:null
    const NW=152,NH=72,PY=20,WY=PY+NH+64,SY=WY+(NH+10)+60,OY=SY+NH+10+60
    ...
    return(
      <div className="flex-1 flex flex-col" style={{minHeight:0}}>
        ...
        <div className="flex-1 overflow-y-auto px-2 pb-2" style={{minHeight:0}}>
          <svg width="100%" viewBox={`0 0 ${W} ${TOTAL}`} preserveAspectRatio="xMidYMid meet" style={{display:"block",width:"100%",minHeight:Math.min(TOTAL,600)}}>
            <defs><AD id="ag" stroke="#d4d4d8"/><AD id="agg" stroke={T.green}/></defs>
        ...
        <div className="flex items-center gap-1 px-4 py-2 flex-shrink-0">
          {[Maximize2,Plus,Minus].map((Icon,i)=><button key={i} type="button" className="w-7 h-7 rounded-lg flex items-center justify-center hover:bg-zinc-100 transition-colors" style={{border:`1px solid ${T.border2}`,background:T.bg}}><Icon style={{width:11,height:11,color:T.text3}}/></button>)}
        </div>
      </div>
    )
  }
  const N=agents.length,gap=Math.min(140,(W-60)/Math.max(N,1)),xs=agents.map((_,i)=>(W-(N-1)*gap)/2+i*gap)
    ...
    <div className="flex-1 flex flex-col" style={{minHeight:0}}>
      ...
      <div className="flex-1 overflow-auto px-2 pb-2" style={{minHeight:0}}>
        <svg width="100%" viewBox={`0 0 ${W} ${TOTAL}`} style={{display:"block",minHeight:TOTAL}}>
          <defs><AD id="ag" stroke="#d4d4d8"/></defs>
...
const LiveExecution = ({agents,running,result,onTrace}:{agents:Agent[];running:boolean;result:SwarmResult|null;onTrace:()=>void}) => {
  ...
  return(
    <div className="flex-shrink-0" style={{borderTop:`1px solid ${T.border}`,background:T.bg}}>
      <div className="flex items-center gap-2.5 px-5 py-2" style={{borderBottom:`1px solid ${T.border}`}}>
        {running?(<><span className="w-2 h-2 rounded-full animate-pulse flex-shrink-0" style={{background:T.green}}/><span style={{fontSize:11,fontWeight:700,color:T.text}}>Live Execution</span><span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold" style={{background:T.greenBg,color:T.greenTx,border:`1px solid ${T.greenBd}`}}>In Progress</span><span style={{fontSize:10,color:T.text3}}>Started {startedAt} · Elapsed {fmt(elapsed)}</span></>
        ):(<><CheckCircle2 style={{width:13,height:13,color:T.green,flexShrink:0}}/><span style={{fontSize:11,fontWeight:700,color:T.text}}>Execution Complete</span></>)}
        <button onClick={onTrace} className="ml-auto flex items-center gap-1.5 hover:underline" style={{fontSize:10,color:T.brand,fontWeight:600}}>
          <Eye style={{width:11,height:11}}/> View Full Trace
        </button>
        <div className="flex ml-3">{[0,15,30,45,60].map(t=>(<span key={t} style={{width:52,textAlign:"center",fontSize:9,color:T.text4,flexShrink:0}}>{t}s</span>))}</div>
      </div>
      {steps.map(s=>(
        <div key={s.num} className="flex items-center gap-2.5 px-5 py-[6px]" style={{borderBottom:"1px solid rgba(240,240,241,0.8)"}}>
...
const RightPanel = ({agents,mode,rounds,running,result,sessions,templates,onCreateV2,onOpenTemplates}:{agents:Agent[];mode:SwarmMode;rounds:number;running:boolean;result:SwarmResult|null;sessions:any[];templates:Template[];onCreateV2:()=>void;onOpenTemplates:()=>void}) => {
  ...
  return(
    <aside className="flex-shrink-0 overflow-y-auto" style={{width:280,borderLeft:`1px solid ${T.border}`,background:T.bg}}>
      <div className="px-4 pt-4 pb-3" style={{borderBottom:`1px solid ${T.border}`}}>
        <div className="flex items-center gap-2 mb-3"><Brain style={{width:14,height:14,color:T.brand}}/><span style={{fontSize:13,fontWeight:700,color:T.text}}>Swarm Intelligence</span></div>
        <div className="grid grid-cols-2 gap-2 mb-3">
          {[
            ...
            {label:"Complexity",v:has?m.complexity:"—",spark:false,vc:!has?undefined:m.complexity==="High"?T.red:m.complexity==="Medium"?T.amber:T.green},
          ].map((k,i)=>(
        ...
        {[{Icon:Cpu,label:"Models Used",value:has?m.models:"—"},{Icon:Zap,label:"Parallel Workers",value:has?`${m.workers} agents`:"—"},{Icon:GitBranch,label:"Routing Strategy",value:"Cost-aware · Confidence-based"},{Icon:MemoryStick,label:"Memory",value:"Enabled (Long-term)"}].map(({Icon,label,value})=>(
          <div key={label} className="flex items-center gap-2 py-[5px]" style={{borderBottom:"1px solid rgba(240,240,241,0.9)"}}>
      ...
      <AnimatePresence>
        {result&&(
          <motion.div initial={{opacity:0,height:0}} animate={{opacity:1,height:"auto"}} exit={{opacity:0,height:0}} className="overflow-hidden" style={{borderBottom:`1px solid ${T.border}`}}>
              ...
              <div className="flex gap-3">
                <div className="relative flex-shrink-0" style={{width:52,height:52}}>
                  <svg width="52" height="52" viewBox="0 0 52 52">
                    ...
                    <circle cx="26" cy="26" r="20" fill="none" stroke={T.green} strokeWidth="7" strokeDasharray="115.6 125.7" strokeLinecap="round" transform="rotate(-90 26 26)"/>
                  </svg>
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {["Strong research depth","High source reliability","Well-structured output"].map(s=>(
                    <div key={s} className="flex items-center gap-1.5 mb-[3px]"><Check style={{width:9,height:9,color:T.green,flexShrink:0,strokeWidth:3}}/><span style={{fontSize:10,color:T.text1}}>{s}</span></div>
                  ))}
                  ...
                  {["Add Legal Analyst agent","Include more risk analysis","Add industry expert review"].map(s=>(
                    <div key={s} className="flex items-center gap-1.5 mb-[3px]"><Lightbulb style={{width:9,height:9,color:T.amber,flexShrink:0}}/><span style={{fontSize:10,color:T.text1}}>{s}</span></div>
                  ))}
                </div>
              </div>
              <button onClick={onCreateV2} className="mt-3 w-full flex items-center justify-center gap-1.5 rounded-xl font-bold transition-all hover:opacity-90"
                style={{height:32,background:"#18181b",color:"#fff",fontSize:11,border:"none",cursor:"pointer"}}>
      ...
      <div className="pt-3 pb-2" style={{borderBottom:`1px solid ${T.border}`}}>
        <div className="flex items-center justify-between px-4 mb-2"><p style={{fontSize:12,fontWeight:700,color:T.text}}>Recent Swarms</p><button style={{fontSize:10,color:T.brand,fontWeight:600}} className="hover:underline">View all</button></div>
        {list.slice(0,5).map((s:any,i:number)=>{
          ...
          return(
            <button key={i} type="button" className="w-full flex items-center gap-2.5 px-4 py-[6px] hover:bg-zinc-50 transition-colors text-left group">
              <div className="w-[22px] h-[22px] rounded-full flex items-center justify-center flex-shrink-0" style={{background:ok?T.greenBg:T.redBg}}>
              ...
              <span className="flex-shrink-0 text-[10px] font-bold px-1.5 py-0.5 rounded-full" style={{background:ok?T.greenBg:T.redBg,color:ok?T.greenTx:T.redTx}}>{ok?"Completed":"Failed"}</span>
            </button>
          )
        })}
      </div>
      <div className="pt-3 pb-4">
        <div className="flex items-center justify-between px-4 mb-2"><p style={{fontSize:12,fontWeight:700,color:T.text}}>Saved Templates</p><button onClick={onOpenTemplates} style={{fontSize:10,color:T.brand,fontWeight:600}} className="hover:underline">View all</button></div>
        {tmplNames.map((t:string)=>(
          <button key={t} type="button" className="w-full flex items-center gap-2.5 px-4 py-[5px] hover:bg-zinc-50 transition-colors text-left">
...
const AgentRow = ({agent,idx,selected,onToggle}:{agent:Agent;idx:number;selected:boolean;onToggle:()=>void}) => {
  ...
  return(
    <button type="button" onClick={onToggle}
      ...
      <div className="w-[30px] h-[30px] rounded-[9px] flex items-center justify-center flex-shrink-0" style={{background:meta.bg,border:`1.5px solid ${meta.bd}`}}>
      ...
      <div className="flex-1 min-w-0">
        ...
        <p style={{fontSize:9.5,lineHeight:1,marginTop:2,color:meta.color,fontWeight:600}}>Role: {meta.role}<span style={{color:T.text3,fontWeight:400}}> · {shortModel(agent.model_name)}</span></p>
      </div>
...
export default function SwarmClient() {
  ...
  const runSwarm=useCallback(async()=>{
    ...
    try{
      ...
      const msg=err.message??"Swarm execution failed";setError(msg);toast.error(msg)
  ...
  return(
    <div className="-mx-6 -my-8 flex flex-col" style={{height:"100vh",minHeight:720,overflow:"hidden",background:T.bg}}>
      ...
      {/* Top Bar */}
      <header className="flex items-center gap-3 flex-shrink-0" style={{padding:"10px 20px",borderBottom:`1px solid ${T.border}`,minHeight:52,background:T.bg,boxShadow:"0 1px 0 rgba(0,0,0,0.03)"}}>
        <div>
          <h1 style={{fontSize:17,fontWeight:700,letterSpacing:-0.4,color:T.text,lineHeight:1.2}}>Multi-Agent Swarm</h1>
          <p style={{fontSize:11,color:T.text3,marginTop:1}}>Build, visualize, and execute intelligent agent teams.</p>
        </div>
        <div className="ml-auto flex items-center gap-2">
          <button type="button" onClick={()=>setMTemplates(true)} className="flex items-center gap-1.5 rounded-[10px] font-semibold transition-all hover:bg-zinc-50 active:scale-95" style={{height:32,padding:"0 12px",border:`1px solid ${T.border2}`,fontSize:11,color:T.text1,background:T.bg}}>
            <LayoutTemplate style={{width:13,height:13}}/> Templates
          </button>
          <button type="button" onClick={()=>setMSave(true)} className="flex items-center gap-1.5 rounded-[10px] font-semibold transition-all hover:bg-zinc-50 active:scale-95" style={{height:32,padding:"0 12px",border:`1px solid ${T.border2}`,fontSize:11,color:T.text1,background:T.bg}}>
          ...
          <button type="button" onClick={runSwarm} disabled={!canLaunch} aria-busy={running}
            ...
            style={{height:32,padding:"0 16px",fontSize:12,color:"#fff",border:"none",cursor:canLaunch?"pointer":"default",background:canLaunch?T.brand:`rgba(99,102,241,0.35)`,boxShadow:canLaunch?"0 2px 12px rgba(99,102,241,0.30)":"none"}}>
            {running?<><Loader2 style={{width:13,height:13}} className="animate-spin"/> Running…</>:<><Play style={{width:13,height:13}}/> Launch Swarm ▶</>}
          </button>
        </div>
      </header>
      ...
      <div className="flex flex-1 overflow-hidden">
        {/* Left Panel */}
        <aside className="flex-shrink-0 flex flex-col overflow-y-auto" style={{width:272,borderRight:`1px solid ${T.border}`,background:T.bg}}>
          {/* Mode cards */}
          <div style={{padding:"10px",borderBottom:`1px solid ${T.border}`}}>
            <div className="grid grid-cols-3 gap-1.5">
              {MODES.map(m=>(
                <button key={m.id} type="button" onClick={()=>setMode(m.id)}
                  className="relative flex flex-col items-center text-center transition-all rounded-xl focus:outline-none active:scale-95"
                  style={{padding:"9px 6px 8px",border:mode===m.id?`2px solid ${m.bdr}`:`1.5px solid ${T.border}`,background:mode===m.id?m.bg:T.soft}}>
                  {mode===m.id&&<div className="absolute -top-[6px] -right-[6px] w-[15px] h-[15px] rounded-full flex items-center justify-center z-10" style={{background:m.color}}><Check style={{width:8,height:8,color:"#fff",strokeWidth:3.5}}/></div>}
                  <div className="w-9 h-9 rounded-[9px] flex items-center justify-center text-[17px] mb-1.5" style={{background:mode===m.id?`${m.color}18`:T.surf}}>{m.icon}</div>
                  <p style={{fontSize:10.5,fontWeight:700,lineHeight:1,marginBottom:2,color:mode===m.id?m.color:T.text2}}>{m.label}</p>
          ...
          <div style={{padding:"12px",borderBottom:`1px solid ${T.border}`}}>
            <p style={{fontSize:10,fontWeight:700,textTransform:"uppercase",letterSpacing:"0.07em",color:T.text2,marginBottom:6}}>Task</p>
            <textarea value={task} onChange={e=>setTask(e.target.value)} rows={4}
              ...
              style={{fontSize:12,lineHeight:1.55,padding:"8px 10px",border:`1px solid ${T.border2}`,color:T.text,background:T.bg,fontFamily:"inherit"}}
              onFocus={e=>(e.target.style.borderColor=T.brand)} onBlur={e=>(e.target.style.borderColor=T.border2)}/>
            {files.length>0&&(
              <div className="flex flex-wrap gap-1.5 mt-2">
                {files.map(f=>(
                  <div key={f.id} className="flex items-center gap-1 rounded-lg pl-1.5 pr-1 py-1 group" style={{background:T.surf,border:`1px solid ${T.border}`,maxWidth:128}}>
                    {f.preview?<img src={f.preview} className="w-5 h-5 rounded object-cover flex-shrink-0" alt=""/>:f.kind==="doc"?<FileText style={{width:11,height:11,color:T.blue}}/>:<File style={{width:11,height:11,color:T.text3}}/>}
                    <span className="truncate" style={{fontSize:9.5,color:T.text1,maxWidth:70}}>{f.file.name}</span>
                    <button type="button" onClick={()=>setFiles(p=>p.filter(x=>x.id!==f.id))} className="flex-shrink-0 opacity-0 group-hover:opacity-100 transition-opacity p-0.5 rounded hover:bg-red-50">
              ...
              {showCtx&&(
                <motion.div initial={{opacity:0,y:-4}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-4}} transition={{duration:0.12}}
                  ...
                  <div className="px-3 py-2 flex items-center gap-2" style={{borderBottom:`1px solid ${T.border}`}}>
                    <AlignLeft style={{width:11,height:11,color:T.blue}}/><span style={{fontSize:11,fontWeight:700,color:T.text}}>Additional Context</span>
              ...
              {showVars&&(
                <motion.div initial={{opacity:0,y:-4}} animate={{opacity:1,y:0}} exit={{opacity:0,y:-4}} transition={{duration:0.12}}
                  ...
                  <div className="px-3 py-2 flex items-center gap-2" style={{borderBottom:`1px solid ${T.border}`}}>
                    ...
                    <span style={{fontSize:9.5,color:T.text3}}>{OPEN_VAR}name{CLOSE_VAR} in task</span>
                    <button type="button" onClick={addVar} className="ml-auto flex items-center gap-1 px-2 py-0.5 rounded-lg hover:bg-indigo-50 transition-colors" style={{fontSize:10,color:T.brand,fontWeight:600}}>
                      ...
                      <div key={v.id} className="flex items-center gap-1.5">
                        <div className="flex items-center rounded-lg overflow-hidden flex-1" style={{border:`1px solid ${T.border2}`}}>
                          <div className="flex items-center gap-0.5 px-2 py-1" style={{background:T.surf,borderRight:`1px solid ${T.border2}`}}>
                            <span style={{fontSize:10,color:T.text3,fontFamily:"monospace"}}>{OPEN_VAR}</span>
                            ...
                            <span style={{fontSize:10,color:T.text3,fontFamily:"monospace"}}>{CLOSE_VAR}</span>
                          </div>
          ...
          <div style={{padding:"12px",borderBottom:`1px solid ${T.border}`}}>
            {[
              ...
              {label:"Accuracy vs Cost",val:accCost,set:setAccCost,min:0,max:100,step:5,display:accCost<=30?"Faster/Cheaper":accCost>=70?"Higher Accuracy":"Balanced",lo:"Faster",hi:"Accurate"},
            ].map(s=>(
              ...
              <div key={f.label} className="flex items-center gap-2.5" style={{marginBottom:10}}>
                ...
                {(f as any).action&&(
                  <button type="button" onClick={(f as any).action} disabled={(f as any).loading}
                    ...
                    {(f as any).loading?<Loader2 style={{width:10,height:10,color:T.text3}} className="animate-spin"/>:<ChevronRight style={{width:10,height:10,color:T.text3}}/>}
                  </button>
                )}
              </div>
            ))}
          ...
          <div className="flex-1 flex flex-col min-h-0">
            <div className="flex items-center justify-between px-3 py-2" style={{borderBottom:`1px solid ${T.border}`}}>
              <p style={{fontSize:11,fontWeight:700,color:T.text}}>Selected Agents <span style={{fontWeight:400,color:T.text3}}>({selAgents.length})</span></p>
              {selAgents.length>0&&<button type="button" onClick={()=>setSelected([])} style={{fontSize:10,color:T.text3,fontWeight:500}} className="hover:text-zinc-600 transition-colors">Clear all</button>}
            </div>
            {agentsLoading?(
              <div className="flex items-center gap-2 px-4 py-6" style={{color:T.text3,fontSize:12}}><Loader2 style={{width:14,height:14}} className="animate-spin"/> Loading agents…</div>
            ):agents.length===0?(
              <div className="px-4 py-8 text-center space-y-2">
                ...
                <a href="/builder" style={{fontSize:12,color:T.brand}} className="underline font-semibold">Create your first agent →</a>
              </div>
            ):(
        ...
        <main className="flex-1 flex flex-col overflow-hidden min-w-0" style={{background:T.bg}}>
          {error&&(
            <div className="mx-4 mt-3 flex items-start gap-2.5 rounded-xl p-3 flex-shrink-0" style={{background:T.redBg,border:`1px solid ${T.redBd}`}}>
              ...
              <button type="button" onClick={()=>setError(null)} className="flex-shrink-0"><X style={{width:13,height:13,color:T.redTx}}/></button>
            </div>
          )}
          ...
          <div className="flex flex-1 overflow-hidden">
            <div className="flex-1 flex flex-col overflow-hidden">
              <div className="flex items-center gap-2 px-4 py-2.5 flex-shrink-0" style={{borderBottom:`1px solid ${T.border}`}}>
                ...
                {running&&<span className="inline-flex items-center gap-1 px-2 py-0.5 rounded-full text-[10px] font-bold animate-pulse" style={{background:T.blueBg,color:T.blue,border:`1px solid ${T.blueBd}`}}>● Live</span>}
                {result&&<span className="inline-flex items-center px-2 py-0.5 rounded-full text-[10px] font-bold" style={{background:T.greenBg,color:T.greenTx,border:`1px solid ${T.greenBd}`}}>✓ Complete</span>}
              </div>
            ...
            {/* Debate Settings */}
            {mode==="debate"&&(
              <aside className="flex-shrink-0 overflow-y-auto" style={{width:208,borderLeft:`1px solid ${T.border}`,padding:14}}>
                ...
                <div style={{marginBottom:14}}>
                  ...
                  {CONSENSUS.map(method=>(
                    <label key={method} className="flex items-center gap-2 cursor-pointer mb-2" onClick={()=>setCm(method)}>
                      <div className="w-3.5 h-3.5 rounded-full border-2 flex items-center justify-center flex-shrink-0 transition-all" style={{borderColor:cm===method?T.blue:T.border3,background:cm===method?T.blue:T.bg}}>
                ...
                <div style={{marginBottom:14}}>
                  ...
                  <div className="relative">
                    <select value={arbiter} onChange={e=>setArbiter(e.target.value)} className="w-full appearance-none focus:outline-none transition-all"
                      style={{height:28,fontSize:10.5,borderRadius:7,border:`1px solid ${T.border2}`,paddingLeft:8,paddingRight:22,color:T.text,background:T.bg}}
                      onFocus={e=>(e.currentTarget.style.borderColor=T.brand)} onBlur={e=>(e.currentTarget.style.borderColor=T.border2)}>
                ...
                <div style={{marginBottom:14}}>
                  ...
                  <div className="relative">
                    <select value={conflictR} onChange={e=>setConflictR(e.target.value)} className="w-full appearance-none focus:outline-none transition-all"
                      style={{height:28,fontSize:10.5,borderRadius:7,border:`1px solid ${T.border2}`,paddingLeft:8,paddingRight:22,color:T.text,background:T.bg}}
                      onFocus={e=>(e.currentTarget.style.borderColor=T.brand)} onBlur={e=>(e.currentTarget.style.borderColor=T.border2)}>
          ...
          <AnimatePresence>
            {result?.finalAnswer&&(
              <motion.div initial={{opacity:0,y:6}} animate={{opacity:1,y:0}} exit={{opacity:0}}
                className="flex-shrink-0 border-t" style={{borderColor:T.border,padding:"14px 18px",background:T.bg}}>
                ...
                <div className="overflow-y-auto rounded-xl" style={{maxHeight:160,background:T.soft,padding:12,fontSize:12,lineHeight:1.65,color:T.text1,whiteSpace:"pre-wrap",border:`1px solid ${T.border}`}}>
                  {result.finalAnswer}
                </div>
              </motion.div>
            )}
