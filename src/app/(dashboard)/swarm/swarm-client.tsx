...
/**
 * AgentDyne — Multi-Agent Swarm
 * Redesigned to Google/Apple UI standards:
 *   - Clean spatial hierarchy, consistent 8px grid
 *   - SF Pro-style typography: tight tracking, clear weight ladder
 *   - Graph: bezier curves, animated flow, gradient nodes, confidence rings
 *   - Zero emoji — all decoration via Lucide icons
 *   - Framer Motion micro-interactions throughout
 *   - Inline styles only for truly dynamic values; Tailwind for everything static
 */
...
import {
  ...
  GripVertical, MoreHorizontal, Plus, Cpu,
  Paperclip, AlignLeft, X, FileText, File, Hash, Crown, Rocket,
  ArrowRight, Clock, DollarSign, Target, List,
  Layers, MessageSquare, Activity, Settings2, TrendingUp,
  Shield, AlertTriangle, Info, CheckCircle, Minus,
} from "lucide-react"
...
/* ─── Role mapping ───────────────────────────────────────────────────── */
type RoleMeta = { role:string; color:string; bg:string; bd:string; grad:string }
const ROLE_MAP: [string, RoleMeta][] = [
  ["research",  { role:"Research",     color:C.brand,  bg:C.brandBg,  bd:C.brandBd,  grad:"135deg,#eef2ff,#e0e7ff" }],
  ["analyst",   { role:"Analysis",     color:C.blue,   bg:C.blueBg,   bd:C.blueBd,   grad:"135deg,#eff6ff,#dbeafe" }],
  ["financial", { role:"Analysis",     color:C.blue,   bg:C.blueBg,   bd:C.blueBd,   grad:"135deg,#eff6ff,#dbeafe" }],
  ["writer",    { role:"Synthesis",    color:C.violet, bg:C.violetBg, bd:C.violetBd, grad:"135deg,#f5f3ff,#ede9fe" }],
  ["critic",    { role:"Critic",       color:C.amber,  bg:C.amberBg,  bd:C.amberBd,  grad:"135deg,#fffbeb,#fef3c7" }],
  ["checker",   { role:"Verification", color:C.amber,  bg:C.amberBg,  bd:C.amberBd,  grad:"135deg,#fffbeb,#fef3c7" }],
  ["reviewer",  { role:"Review",       color:C.teal,   bg:C.tealBg,   bd:C.tealBd,   grad:"135deg,#f0fdfa,#ccfbf1" }],
  ["planner",   { role:"Planning",     color:C.brand,  bg:C.brandBg,  bd:C.brandBd,  grad:"135deg,#eef2ff,#e0e7ff" }],
  ["coder",     { role:"Engineering",  color:C.green,  bg:C.greenBg,  bd:C.greenBd,  grad:"135deg,#f0fdf4,#dcfce7" }],
]
const DEF_ROLE: RoleMeta = { role:"General", color:C.text2, bg:C.surf, bd:C.border2, grad:`135deg,${C.surf},${C.surf}` }
...
const Toggle = ({ on, set }: { on:boolean; set:(v:boolean)=>void }) => (
  <button type="button" role="switch" aria-checked={on} onClick={() => set(!on)}
    className="relative flex-shrink-0 rounded-full transition-colors duration-200 focus-visible:ring-2 focus-visible:ring-indigo-500"
    style={{ width:36, height:20, background: on ? C.brand : C.border3 }}>
    <span className="absolute top-[2px] rounded-full bg-white shadow-md transition-transform duration-200"
...
const CB = ({ on, set }: { on:boolean; set:(v:boolean)=>void }) => (
  <button type="button" role="checkbox" aria-checked={on} onClick={() => set(!on)}
    className="flex items-center justify-center flex-shrink-0 rounded-md transition-all duration-150 focus-visible:ring-2 focus-visible:ring-indigo-500"
      ...
      {open && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          ...
          <motion.div initial={{ opacity:0, scale:0.95, y:12 }} animate={{ opacity:1, scale:1, y:0 }}
            exit={{ opacity:0, scale:0.95 }} transition={{ duration:0.16, ease:[0.25,0.1,0.25,1] }}
            className="relative flex flex-col overflow-hidden"
            style={{ background:C.bg, width:"100%", maxWidth:width, maxHeight:"90vh", borderRadius:20,
              boxShadow:"0 32px 80px rgba(0,0,0,0.2), 0 0 0 1px rgba(0,0,0,0.06)" }}>
            <div className="flex items-center justify-between px-5 py-4 flex-shrink-0"
              style={{ borderBottom:`1px solid ${C.border}` }}>
              <p className="text-sm font-bold" style={{ color:C.text, letterSpacing:-0.3 }}>{title}</p>
              <button onClick={close}
                className="w-7 h-7 rounded-xl flex items-center justify-center transition-colors hover:bg-zinc-100">
    ...
    <Modal open={open} close={close} title="Upgrade to Launch Swarms" width={440}>
      <div className="p-6 space-y-5">
        ...
        <div className="text-center">
          <p className="font-bold text-zinc-900 mb-2" style={{ fontSize:16, letterSpacing:-0.4 }}>Starter plan required</p>
          <p className="text-sm text-zinc-500 leading-relaxed max-w-xs mx-auto">
            Multi-agent swarms require the Starter plan. Coordinate up to 8 agents, unlock all modes, and build persistent swarm memory.
          </p>
        </div>
        ...
        <div className="flex gap-3">
          <button onClick={close}
            className="flex-1 h-11 rounded-xl font-semibold text-sm text-zinc-600 transition-colors hover:bg-zinc-50"
            style={{ border:`1px solid ${C.border2}` }}>
            Maybe later
          </button>
          <button onClick={() => { close(); router.push("/billing") }}
            className="flex-1 h-11 rounded-xl font-bold text-sm text-white flex items-center justify-center gap-2 transition-opacity hover:opacity-90"
  ...
  return (
    <Modal open={open} close={close} title="Save as Template" width={420}>
      <div className="p-5 space-y-4">
        ...
        {[
          { label:"Template Name *", value:name, set:setName, placeholder:"e.g. Investment Research Swarm", multi:false },
          { label:"Description", value:desc, set:setDesc, placeholder:"What does this swarm do?", multi:true },
        ].map(f => (
          <div key={f.label}>
            <label className="text-xs font-semibold text-zinc-700 block mb-1.5">{f.label}</label>
            {f.multi ? (
              <textarea rows={2} value={f.value} onChange={e => f.set(e.target.value)}
                placeholder={f.placeholder}
                className="w-full rounded-xl resize-none focus:outline-none transition-all text-sm px-3 py-2.5"
                style={{ border:`1px solid ${C.border2}`, fontFamily:"inherit" }} />
            ) : (
              <input value={f.value} onChange={e => f.set(e.target.value)} placeholder={f.placeholder} autoFocus
                className="w-full rounded-xl focus:outline-none transition-all h-9 text-sm px-3"
                style={{ border:`1px solid ${C.border2}` }} />
            )}
          </div>
        ))}
        <div className="flex gap-3 pt-1">
          <button onClick={close}
            className="flex-1 h-10 rounded-xl font-semibold text-sm text-zinc-600 hover:bg-zinc-50 transition-colors"
            style={{ border:`1px solid ${C.border2}` }}>
            Cancel
          </button>
          <button onClick={handle} disabled={saving || !name.trim()}
            className="flex-1 h-10 rounded-xl font-bold text-sm text-white transition-all"
...
const PlanCard = ({ agents, mode, rounds }: { agents:Agent[]; mode:SwarmMode; rounds:number }) => {
  ...
  return (
    <motion.div initial={{ opacity:0, y:-6 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, y:-6 }}
      ...
      <div className="flex items-center justify-between px-4 py-3 flex-shrink-0" style={{ borderBottom:`1px solid ${C.brandBd}` }}>
...
/* ─── Swarm Graph ────────────────────────────────────────────────────── */
// Completely rewritten: bezier curves, gradient fills, animated flow particles,
// confidence arcs, drop shadows, clean legend — matches Apple/Google visual quality.

function bezier(x1:number, y1:number, x2:number, y2:number, curvature=0.45): string {
  const cy = Math.abs(y2 - y1) * curvature
...
function GraphNode({ x, y, w, h, agent, conf, pulse, rx=14 }:{
  ...
  const mid = x + w/2
  const uid = `grad-${agent.id.slice(0,8)}`
  const sid = `shad-${agent.id.slice(0,8)}`
    ...
    <g>
      <defs>
        <linearGradient id={uid} x1="0" y1="0" x2="0" y2="1">
          ...
          <stop offset="100%" stopColor={m.bd} stopOpacity="0.4" />
        </linearGradient>
        <filter id={sid} x="-20%" y="-20%" width="140%" height="140%">
          <feDropShadow dx="0" dy="2" stdDeviation="4" floodColor={m.color} floodOpacity="0.12" />
        </filter>
      </defs>

      {/* Node body */}
      <rect x={x} y={y} width={w} height={h} rx={rx}
        fill={`url(#${uid})`} stroke={m.bd} strokeWidth="1.5"
        filter={`url(#${sid})`} />

      {/* Role pill */}
      <rect x={mid - 30} y={y + 8} width={60} height={16} rx={8}
        fill={m.color} fillOpacity="0.12" />
      <text x={mid} y={y + 19.5} textAnchor="middle" fontSize="8.5" fontWeight="700"
        fill={m.color} fontFamily="system-ui,-apple-system,sans-serif">
        {m.role.toUpperCase()}
      </text>

      {/* Agent name */}
      <text x={mid} y={y + 38} textAnchor="middle" fontSize="11.5" fontWeight="700"
        fill="#18181b" fontFamily="system-ui,-apple-system,sans-serif">
        {agent.name.slice(0, 16)}{agent.name.length > 16 ? "…" : ""}
      </text>

      {/* Model badge */}
      <rect x={mid - 22} y={y + 44} width={44} height={14} rx={7}
        fill="#f4f4f5" stroke="#e4e4e7" strokeWidth="1" />
      <text x={mid} y={y + 54} textAnchor="middle" fontSize="8" fontWeight="600"
        fill="#71717a" fontFamily="system-ui,-apple-system,sans-serif">
        {shortModel(agent.model_name)}
      </text>

      {/* Confidence arc */}
      {conf !== undefined && (() => {
        const r = 10, cx = x + w - 18, cy2 = y + h - 16
        const pct = conf / 100
        const θ = pct * 2 * Math.PI - Math.PI / 2
        const lx = cx + r * Math.cos(θ), ly = cy2 + r * Math.sin(θ)
        const large = pct > 0.5 ? 1 : 0
        return (
          <g>
            <circle cx={cx} cy={cy2} r={r} fill="none" stroke={m.bd} strokeWidth="2.5" />
            <path d={`M${cx},${cy2 - r} A${r},${r} 0 ${large},1 ${lx},${ly}`}
              fill="none" stroke={m.color} strokeWidth="2.5" strokeLinecap="round" />
            <text x={cx} y={cy2 + 3.5} textAnchor="middle" fontSize="7.5" fontWeight="700" fill={m.color}
              fontFamily="system-ui,-apple-system,sans-serif">{conf}%</text>
          </g>
        )
      })()}

      {/* Pulse bar when running */}
      {pulse && (
        <rect x={x + 8} y={y + h - 5} height={3} rx={1.5} fill={m.color} opacity="0.4">
...
// Animated flow particle along a bezier path
function FlowParticle({ path, color, delay=0, dur=2 }:{ path:string; color:string; delay?:number; dur?:number }) {
  return (
...
const SwarmGraph = ({ agents, mode, running }: { agents:Agent[]; mode:SwarmMode; running:boolean }) => {
  ...
  const LEGEND = [
    { c:C.brand,  l:"Orchestrator" },
    { c:C.blue,   l:"Analysis"     },
    { c:C.violet, l:"Synthesis"    },
    { c:C.green,  l:"Output"       },
  ]
  ...
  if (mode === "orchestrate" && agents.length >= 2) {
    ...
    const PY = 24
    const WY = PY + NH + 72
    const SY = WY + NH + 10 + 72
    const OY = SY + NH + 10 + 60
    const TOTAL = (synth ? OY + 56 : WY + NH + 20 + 60) + 20

    const N   = workers.length
    const gap = Math.min(180, (W - 40) / Math.max(N, 1))
      ...
      <div className="flex-1 flex flex-col" style={{ minHeight:0 }}>
        <div className="flex items-center gap-5 px-5 py-2.5 flex-shrink-0 flex-wrap"
          ...
          {LEGEND.map(l => (
            <div key={l.l} className="flex items-center gap-1.5">
              ...
              <span className="text-xs text-zinc-400 font-medium">{l.l}</span>
            </div>
          ))}
          <span className="ml-auto text-xs font-semibold text-zinc-400">
            {N + (synth ? 2 : 1)} agents · orchestrate
          </span>
        </div>
        <div className="flex-1 overflow-y-auto px-2 pb-4" style={{ minHeight:0 }}>
          <svg width="100%" viewBox={`0 0 ${W} ${TOTAL}`} preserveAspectRatio="xMidYMid meet"
            style={{ display:"block", width:"100%", minHeight: Math.min(TOTAL, 580) }}>
            <defs>
              <marker id="arrowGray" viewBox="0 0 8 8" refX="6" refY="4" markerWidth="5" markerHeight="5" orient="auto">
                <path d="M1 1.5L6 4L1 6.5" fill="none" stroke="#d4d4d8" strokeWidth="1.5" strokeLinecap="round"/>
              </marker>
              <marker id="arrowGreen" viewBox="0 0 8 8" refX="6" refY="4" markerWidth="5" markerHeight="5" orient="auto">
            ...
            {/* Planner → Worker edges */}
            {wxs.map((wx, i) => {
              const p = bezier(px, py_bot, wx, WY)
              return (
                <g key={i}>
                  <path d={p} fill="none" stroke="#e4e4e7" strokeWidth="1.5" markerEnd="url(#arrowGray)"
                    strokeDasharray={running ? "5 3" : "none"} />
                  {running && <FlowParticle path={p} color={C.brand} delay={i * 0.4} dur={1.8} />}
                </g>
              )
            })}
            ...
            {/* Worker → Synth edges */}
            {synth && wxs.map((wx, i) => {
              const p = bezier(wx, WY + NH + 10, (W - NW) / 2 + NW / 2, SY)
              return (
                <g key={i}>
                  <path d={p} fill="none" stroke="#e4e4e7" strokeWidth="1.5" markerEnd="url(#arrowGray)" />
                  {running && <FlowParticle path={p} color={C.violet} delay={0.8 + i * 0.3} dur={2.1} />}
                </g>
              )
            })}
            ...
            {/* Synth → Output */}
            {synth && (() => {
              const sx = (W - NW) / 2 + NW / 2
              const p  = bezier(sx, SY + NH + 10, W / 2, OY)
              return (
                <g>
                  <path d={p} fill="none" stroke={C.green} strokeWidth="2" markerEnd="url(#arrowGreen)" />
                  {running && <FlowParticle path={p} color={C.green} delay={1.4} dur={1.6} />}
                </g>
              )
            })()}
                ...
                <g>
                  <defs>
                    <linearGradient id="outGrad" x1="0" y1="0" x2="0" y2="1">
                      <stop offset="0%" stopColor={C.greenBg} />
                      <stop offset="100%" stopColor={C.greenBd} stopOpacity="0.5" />
                    </linearGradient>
                    <filter id="outGlow">
                      <feDropShadow dx="0" dy="3" stdDeviation="6" floodColor={C.green} floodOpacity="0.2" />
                    </filter>
                  </defs>
                  <rect x={(W-176)/2} y={oy} width={176} height={52} rx={14}
                    fill="url(#outGrad)" stroke={C.green} strokeWidth="2" filter="url(#outGlow)" />
                  <text x={W/2} y={oy+23} textAnchor="middle" fontSize="12.5" fontWeight="700" fill={C.greenTx}
                    fontFamily="system-ui,-apple-system,sans-serif">Executive Report</text>
                  <text x={W/2} y={oy+38} textAnchor="middle" fontSize="9.5" fontWeight="500" fill="#86efac"
                    fontFamily="system-ui,-apple-system,sans-serif">Final Output · Synthesised</text>
                </g>
              )
            })()}
          </svg>
  ...
  const gap = Math.min(150, (W - 60) / Math.max(N, 1))
    ...
    <div className="flex-1 flex flex-col" style={{ minHeight:0 }}>
      ...
      <div className="flex-1 overflow-auto px-2 pb-4" style={{ minHeight:0 }}>
        <svg width="100%" viewBox={`0 0 ${W} ${TOTAL}`}
          style={{ display:"block", minHeight:TOTAL }}>
          <defs>
            <marker id="arrowGray2" viewBox="0 0 8 8" refX="6" refY="4" markerWidth="4" markerHeight="4" orient="auto">
              <path d="M1 1.5L6 4L1 6.5" fill="none" stroke="#d4d4d8" strokeWidth="1.5" strokeLinecap="round"/>
            </marker>
          </defs>

          {/* Task box */}
          <rect x={(W-140)/2} y={10} width={140} height={36} rx={12}
            fill={C.brandBg} stroke={C.brandBd} strokeWidth="1.5" />
          <text x={W/2} y={32} textAnchor="middle" fontSize="12" fontWeight="700" fill={C.brand}
          ...
          {/* Fan-out edges */}
          {agents.map((_, i) => {
            const p = bezier(W/2, 46, xs[i], 106)
            return (
              <g key={i}>
                <path d={p} fill="none" stroke="#e4e4e7" strokeWidth="1.5" markerEnd="url(#arrowGray2)" />
                {running && <FlowParticle path={p} color={C.brand} delay={i * 0.35} dur={1.5} />}
              </g>
            )
          })}
          ...
          {agents.map((a, i) => {
            ...
            return (
              <g key={a.id}>
                ...
                <rect x={xs[i]-NW/2} y={110} width={NW} height={NH} rx={14}
                  fill={`url(#${uid})`} stroke={m.bd} strokeWidth="1.5" />
                <rect x={xs[i]-26} y={118} width={52} height={16} rx={8}
                  fill={m.color} fillOpacity="0.12" />
                <text x={xs[i]} y={129} textAnchor="middle" fontSize="8.5" fontWeight="700" fill={m.color}
                  fontFamily="system-ui,-apple-system,sans-serif">{m.role.toUpperCase()}</text>
                <text x={xs[i]} y={147} textAnchor="middle" fontSize="11.5" fontWeight="700" fill="#18181b"
                  fontFamily="system-ui,-apple-system,sans-serif">{a.name.slice(0,14)}</text>
                <text x={xs[i]} y={162} textAnchor="middle" fontSize="8.5" fontWeight="500" fill="#71717a"
                  fontFamily="system-ui,-apple-system,sans-serif">{shortModel(a.model_name)}</text>
                <text x={xs[i]} y={180} textAnchor="middle" fontSize="8.5" fontWeight="700" fill={m.color}
                  fontFamily="system-ui,-apple-system,sans-serif">Conf: {88 + i * 2}%</text>
              </g>
            )
          })}
          ...
          {/* Fan-in edges */}
          {agents.map((_, i) => {
            const p = bezier(xs[i], 190, W/2, 215)
            return <path key={i} d={p} fill="none" stroke="#e4e4e7" strokeWidth="1.5" markerEnd="url(#arrowGray2)" />
          })}

          {/* Merge box */}
          <defs>
            <linearGradient id="mergeGrad" x1="0" y1="0" x2="0" y2="1">
              <stop offset="0%" stopColor={C.greenBg} />
              <stop offset="100%" stopColor={C.greenBd} stopOpacity="0.5" />
            </linearGradient>
          </defs>
          <rect x={(W-120)/2} y={217} width={120} height={34} rx={12}
            fill="url(#mergeGrad)" stroke={C.green} strokeWidth="2" />
          <text x={W/2} y={238} textAnchor="middle" fontSize="11.5" fontWeight="700" fill={C.greenTx}
            fontFamily="system-ui,-apple-system,sans-serif">Merge Results</text>
        </svg>
      </div>
    </div>
  )
...
const LiveExecution = ({ agents, running, result, onTrace }: {
  ...
  const ACTS: Record<string,string> = {
    ...
    Analysis:"Analysing competitors & financials…",
  ...
  const STATUS_STYLES: Record<StepStatus,{ num:string; pill:string; label:string }> = {
    ...
    <div className="flex-shrink-0" style={{ borderTop:`1px solid ${C.border}`, background:C.bg }}>
      ...
      <div className="flex items-center gap-2.5 px-5 py-2.5" style={{ borderBottom:`1px solid ${C.border}` }}>
        ...
        <div className="flex ml-3">
          {[0,15,30,45,60].map(t => (
            <span key={t} className="text-[9px] text-zinc-300 tabular-nums text-center" style={{ width:52 }}>{t}s</span>
          ))}
        </div>
      </div>
      ...
      {steps.map(s => {
        ...
        return (
          <div key={s.num} className="flex items-center gap-2.5 px-5"
            style={{ paddingTop:6, paddingBottom:6, borderBottom:"1px solid rgba(240,240,241,0.7)" }}>
            <span className={cn("w-5 h-5 rounded-full flex items-center justify-center flex-shrink-0 text-[9px] font-bold", st.num)}>
              {s.num}
            </span>
            ...
            <span className={cn("flex-shrink-0 text-[10px] font-bold px-2 py-0.5 rounded-full border", st.pill)}>
              {st.label}
            </span>
            ...
            <div className="w-[104px] h-5 rounded-md flex-shrink-0 relative overflow-hidden" style={{ background:C.surf }}>
              {s.pct > 0 && (
                <div className="absolute top-1.5 h-[10px] rounded-sm transition-all duration-500"
                  style={{
                    left:`${s.bx}%`,
    ...
    <aside className="flex-shrink-0 overflow-y-auto" style={{ width:276, borderLeft:`1px solid ${C.border}`, background:C.bg }}>
      ...
      <div className="px-4 pt-4 pb-3" style={{ borderBottom:`1px solid ${C.border}` }}>
        <div className="flex items-center gap-2 mb-3">
          ...
          <span className="text-sm font-bold text-zinc-900">Swarm Intelligence</span>
        </div>
        ...
        <div className="space-y-0.5">
          {[
            ...
            <div key={label} className="flex items-center gap-2 py-1.5"
              ...
              <Icon style={{ width:11, height:11, color:C.text4, flexShrink:0 }} />
      ...
      <AnimatePresence>
        {result && (
          <motion.div initial={{ opacity:0, height:0 }} animate={{ opacity:1, height:"auto" }}
            exit={{ opacity:0, height:0 }} className="overflow-hidden" style={{ borderBottom:`1px solid ${C.border}` }}>
            <div className="px-4 py-3">
              ...
              <button onClick={onCreateV2}
                className="mt-3 w-full flex items-center justify-center gap-1.5 rounded-xl font-bold text-xs text-white transition-opacity hover:opacity-90"
        ...
        {list.slice(0, 5).map((s:any, i:number) => {
          ...
          return (
            <button key={i} type="button"
              className="w-full flex items-center gap-2.5 px-4 py-1.5 hover:bg-zinc-50 transition-colors text-left">
              <div className="w-6 h-6 rounded-lg flex items-center justify-center flex-shrink-0"
                style={{ background: ok ? C.greenBg : C.redBg }}>
                {ok
                  ? <CheckCircle2 style={{ width:12, height:12, color:C.green }} />
      ...
      <div className="pt-3 pb-4">
        ...
        {tmplNames.map((t:string) => (
          <button key={t} type="button"
            className="w-full flex items-center gap-2.5 px-4 py-1.5 hover:bg-zinc-50 transition-colors text-left">
            <LayoutTemplate style={{ width:12, height:12, color:C.text4, flexShrink:0 }} />
...
/* ─── Agent row ──────────────────────────────────────────────────────── */
const AgentRow = ({ agent, idx, selected, onToggle }: {
...
const MODES = [
  ...
  { id:"parallel"    as SwarmMode, icon:Activity,       label:"Parallel",    desc:"Simultaneous for maximum speed",      color:C.amber,  bg:C.amberBg,  bdr:C.amberBd  },
]
...
export default function SwarmClient() {
  ...
  const saveTemplate = useCallback(async (name:string, desc:string) => {
    ...
    const { error:err } = await supabase.from("swarm_templates").insert({
      owner_id:user.id, name, description:desc, mode, agent_roles:[],
      config:{ rounds, cm, budget, maxRT, accCost }, is_public:false,
    })
    ...
    <div className="-mx-6 -my-8 flex flex-col" style={{ height:"100vh", minHeight:720, overflow:"hidden", background:C.bg }}>
      ...
      <header className="flex items-center gap-3 flex-shrink-0"
        ...
        {selAgents.length > 0 && (
          <div className="flex items-center gap-1.5 ml-2">
            ...
            {selAgents.length > 4 && (
              <span className="text-[10px] font-bold text-zinc-400">+{selAgents.length - 4}</span>
            )}
          </div>
        )}
        ...
        <div className="ml-auto flex items-center gap-2">
          {[
            { label:"Templates",       icon:LayoutTemplate, click:() => setMTemplates(true) },
            { label:"Save Template",   icon:Save,           click:() => setMSave(true)      },
          ...
          <button type="button" onClick={runSwarm} disabled={!canLaunch}
            ...
            style={{
              ...
              background: canLaunch ? `linear-gradient(135deg,${C.brand},${C.brandDk})` : `${C.brand}40`,
              boxShadow: canLaunch ? `0 2px 12px ${C.brand}40` : "none",
              cursor: canLaunch ? "pointer" : "default",
            }}>
            {running
              ? <><Loader2 style={{ width:12, height:12 }} className="animate-spin" /> Running…</>
              : <><Play style={{ width:12, height:12 }} /> Launch Swarm</>}
          </button>
        </div>
      </header>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <aside className="flex-shrink-0 flex flex-col overflow-y-auto"
          ...
          {/* Mode selector — Lucide icons replace emojis */}
              ...
              {MODES.map(m => {
                ...
                return (
                  <button key={m.id} type="button" onClick={() => setMode(m.id)}
                    className="relative flex flex-col items-center text-center transition-all rounded-2xl focus-visible:ring-2 focus-visible:ring-indigo-500 active:scale-95"
                    style={{
                      padding:"10px 6px 9px",
                      border: active ? `2px solid ${m.bdr}` : `1.5px solid ${C.border}`,
                      background: active ? m.bg : C.soft,
                    }}>
                    {active && (
          ...
          <div style={{ padding:"12px", borderBottom:`1px solid ${C.border}` }}>
            ...
            <div className="flex items-center gap-0.5 mt-2">
              <input ref={fileRef} type="file" multiple className="hidden"
                accept="image/*,.pdf,.doc,.docx,.txt,.csv,.xlsx,.json,.md" onChange={onFileChange} />
              {[
              ...
              {showCtx && (
                <motion.div initial={{ opacity:0, y:-4 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, y:-4 }}
                  ...
                  <div className="px-3 py-2 flex items-center gap-2" style={{ borderBottom:`1px solid ${C.border}` }}>
                    ...
                    <span className="text-xs font-bold text-zinc-900">Additional Context</span>
              ...
              {showVars && (
                <motion.div initial={{ opacity:0, y:-4 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0, y:-4 }}
                  ...
                  <div className="px-3 py-2 flex items-center gap-2" style={{ borderBottom:`1px solid ${C.border}` }}>
                    ...
                    <span className="text-xs font-bold text-zinc-900">Variables</span>
                    <span className="text-[9.5px] text-zinc-400">{OPEN_VAR}name{CLOSE_VAR} in task</span>
                    <button type="button" onClick={addVar}
                      className="ml-auto flex items-center gap-1 px-2 py-0.5 rounded-lg hover:bg-indigo-50 transition-colors text-[10px] font-semibold"
                      style={{ color:C.brand }}>
                  ...
                  <div className="p-2 space-y-1.5">
                    {vars.length === 0 && (
                      <p className="text-[10.5px] text-zinc-400 text-center py-1.5">No variables yet. Click Add to create one.</p>
                    )}
                    {vars.map(v => (
                      <div key={v.id} className="flex items-center gap-1.5">
                        <div className="flex items-center rounded-xl overflow-hidden flex-1"
                          ...
                          <div className="flex items-center gap-0.5 px-2 py-1"
                            style={{ background:C.surf, borderRight:`1px solid ${C.border2}` }}>
                            ...
                            <input value={v.key} onChange={e => updVar(v.id, "key", e.target.value)}
                              placeholder="name" className="focus:outline-none bg-transparent font-mono text-[11px] font-semibold"
                              style={{ width:52, color:C.brand }} />
                          ...
                          <input value={v.value} onChange={e => updVar(v.id, "value", e.target.value)}
                            placeholder="value" className="flex-1 focus:outline-none px-2 py-1 text-[11px]"
                    ...
                    {vars.length > 0 && (
                      <button type="button" onClick={applyVars}
                        className="w-full flex items-center justify-center gap-1.5 rounded-xl py-1.5 transition-colors text-[10.5px] font-semibold"
                        style={{ background:`${C.brand}10`, color:C.brand }}>
          ...
          <div style={{ padding:"12px", borderBottom:`1px solid ${C.border}` }}>
            {[
              ...
              { label:"Accuracy vs Cost", val:accCost, set:setAccCost, min:0, max:100, step:5,
                display:accCost <= 30 ? "Faster/Cheaper" : accCost >= 70 ? "Higher Accuracy" : "Balanced", lo:"Faster", hi:"Accurate" },
            ].map(s => (
          ...
          <div style={{ padding:"12px", borderBottom:`1px solid ${C.border}` }}>
            {[
              { label:"Auto Assemble",      desc:"AI selects the best agents for your task", val:autoAsm,  set:setAutoAsm,  action:doAutoAssemble, loading:autoAsmBusy },
              ...
              <div key={f.label} className="flex items-center gap-2.5 mb-3 last:mb-0">
                ...
                {(f as any).action && (
                  <button type="button" onClick={(f as any).action} disabled={(f as any).loading}
                    className="w-6 h-6 rounded-lg flex items-center justify-center transition-colors hover:bg-zinc-100 active:scale-95"
                    style={{ background:C.surf }}>
                    {(f as any).loading
          ...
          <div className="flex-1 flex flex-col min-h-0">
            ...
            {agentsLoading ? (
              <div className="flex items-center gap-2 px-4 py-6 text-zinc-400 text-xs">
              ...
              <div className="px-4 py-8 text-center space-y-2">
                ...
                <a href="/builder" className="text-xs font-semibold underline" style={{ color:C.brand }}>
          ...
          <div className="flex flex-1 overflow-hidden">
            <div className="flex-1 flex flex-col overflow-hidden">
              ...
              <div className="flex items-center gap-2 px-4 py-2.5 flex-shrink-0" style={{ borderBottom:`1px solid ${C.border}` }}>
                ...
                <span className="text-xs font-bold text-zinc-900">Swarm Graph</span>
                {running && (
            ...
            {mode === "debate" && (
              <aside className="flex-shrink-0 overflow-y-auto"
                style={{ width:208, borderLeft:`1px solid ${C.border}`, padding:14 }}>
                ...
                <div className="mb-4">
                  ...
                  <div className="flex items-center gap-2">
                    <input type="number" min={1} max={10} value={rounds}
                      ...
                      className="text-center font-bold focus:outline-none transition-all text-sm"
                ...
                <div className="mb-4">
                  <p className="text-[10px] font-semibold text-zinc-500 mb-1.5">Final Arbiter</p>
                  <div className="relative">
                    <select value={arbiter} onChange={e => setArbiter(e.target.value)}
                      className="w-full appearance-none focus:outline-none text-[10.5px] rounded-xl"
                ...
                <div className="mb-4">
                  <p className="text-[10px] font-semibold text-zinc-500 mb-1.5">Conflict Resolution</p>
                  <div className="relative">
            ...
            {result?.finalAnswer && (
              <motion.div initial={{ opacity:0, y:6 }} animate={{ opacity:1, y:0 }} exit={{ opacity:0 }}
                ...
                <div className="flex items-center gap-2 mb-2.5">
                  ...
                  <p className="text-xs font-bold text-zinc-900 flex-1">Final Answer</p>
                  <button type="button"
                    ...
                    className="flex items-center gap-1 rounded-lg text-[10px] font-semibold transition-colors hover:bg-zinc-100"
