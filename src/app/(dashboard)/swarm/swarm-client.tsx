...
/**
 * Multi-Agent Swarm Dashboard (Gap 4)
 *
 * Enables true peer-to-peer agent communication via:
 *   - Orchestrate mode: agent 1 decomposes, specialists execute, agent 1 synthesises
 *   - Debate mode: agents iterate over each other's proposals
 *   - Parallel mode: all agents work independently, results merged
 *
 * Google A2A-compatible shared context workspace.
...
import {
  Network, Plus, X, Play, Loader2, Check,
  ChevronDown, ChevronUp, Bot, Zap, GitBranch,
  MessageSquare, AlertCircle, Trophy, Copy,
} from "lucide-react"
...
interface SwarmSession {
  ...
  agentCount: number; finalAnswer: string; messageLog: any[]
  rounds: number
}
...
function AgentSelector({ agents, selected, onChange, maxAgents }: {
  ...
  return (
    <div className="space-y-3">
      ...
      {selected.length > 0 && (
        <div className="flex flex-wrap gap-2">
          {selected.map(id => {
            ...
            return (
              <span key={id} className="inline-flex items-center gap-1.5 text-xs font-semibold bg-primary/8 text-primary border border-primary/20 px-2.5 py-1 rounded-full">
      ...
      <div className="grid grid-cols-1 gap-2 max-h-52 overflow-y-auto">
        {filtered.length === 0 && (
          <p className="text-sm text-zinc-400 py-4 text-center">No active agents found. <a href="/builder" className="text-primary underline">Create one</a></p>
        )}
        {filtered.map(a => {
                ...
                <div key={i} className="text-xs">
                  <div className="flex items-center gap-2 mb-1">
                    <span className={cn("font-bold px-2 py-0.5 rounded-full",
                      entry.from === "orchestrator" ? "bg-primary/10 text-primary" :
                      entry.type?.includes("result") ? "bg-green-50 text-green-700" :
                      "bg-zinc-100 text-zinc-600")}>
                      {entry.from ?? entry.type ?? "event"}
                    </span>
                    <span className="text-zinc-300">{entry.timestamp ? new Date(entry.timestamp).toLocaleTimeString() : ""}</span>
                  </div>
                  <pre className="bg-zinc-50 rounded-lg p-2 text-[10px] font-mono text-zinc-600 overflow-x-auto whitespace-pre-wrap">
                    {JSON.stringify(entry.content ?? entry.outputs ?? entry.results ?? entry, null, 2).slice(0, 400)}
                  </pre>
                </div>
              ))}
            </div>
...
export default function SwarmClient() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getUser().then(({ data: { user } }) => {
      ...
      supabase.from("agents").select("id, name, model_name, status")
  ...
  const run = useCallback(async () => {
    ...
    try {
      const res = await fetch("/api/swarm", {
        ...
        body: JSON.stringify({ task: task.trim(), agentIds: selected, name: name.trim() || undefined, mode, maxRounds }),
      })
      const data = await res.json()
      if (!res.ok) throw new Error(data.error ?? `HTTP ${res.status}`)
      setResult(data)
      setSessions(prev => [{ id: data.sessionId, name: name || task.slice(0, 40), status: "completed", created_at: new Date().toISOString() }, ...prev.slice(0, 9)])
          ...
          <div className="grid grid-cols-1 lg:grid-cols-5 gap-6">
            ...
            <div className="lg:col-span-3 space-y-5">
              ...
              <div className="bg-white border border-zinc-100 rounded-2xl p-5 space-y-3" style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                  ...
                  {MODES.map(m => (
                    <button key={m.id} type="button" onClick={() => setMode(m.id)}
                      ...
                      className={cn("w-full flex items-start gap-3 p-3 rounded-xl border text-left transition-all",
                        mode === m.id ? "border-primary/30 bg-primary/5" : "border-zinc-100 hover:border-zinc-200")}>
                      <span className="text-xl flex-shrink-0 mt-0.5">{m.icon}</span>
                      <div>
                        <p className={cn("text-sm font-semibold", mode === m.id ? "text-primary" : "text-zinc-900")}>{m.label}</p>
                ...
                {mode === "debate" && (
                  <div className="flex items-center gap-3 pt-1">
                    <Label className="text-xs font-medium text-zinc-600 flex-shrink-0">Debate rounds</Label>
              ...
              <div className="bg-white border border-zinc-100 rounded-2xl p-5 space-y-3" style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                ...
                <div className="space-y-1.5">
                  <Label className="text-sm font-medium text-zinc-700">Session Name <span className="text-zinc-400 font-normal">(optional)</span></Label>
              ...
              <div className="bg-white border border-zinc-100 rounded-2xl p-5" style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                <Label className="text-sm font-semibold text-zinc-900 block mb-3">
                  Select Agents * <span className="text-zinc-400 font-normal text-xs">(2–8, min 2)</span>
                </Label>
                {loading ? (
                  ...
                  <AgentSelector agents={agents} selected={selected} onChange={setSelected} maxAgents={8} />
                )}
              </div>

              {/* Run button */}
              <Button type="button" onClick={run} disabled={running || selected.length < 2 || !task.trim()}
            ...
            <div className="lg:col-span-2 space-y-4">
              ...
              <AnimatePresence>
                {result && (
                  <motion.div initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }}
                    className="bg-white border border-zinc-100 rounded-2xl overflow-hidden" style={{ boxShadow: "0 4px 20px rgba(0,0,0,0.06)" }}>
                    ...
                    <div className="p-4 space-y-4">
                      <div>
                        <p className="text-[10px] font-bold text-zinc-400 uppercase tracking-wider mb-2">Final Answer</p>
                  ...
                  <div className="flex items-start gap-3 mb-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-zinc-500 mt-1 leading-relaxed">
                        Swarms excel when a single agent can't hold enough context:
                        research + write + review pipelines, multi-domain analysis,
              ...
              {sessions.length > 0 && (
                <div className="bg-white border border-zinc-100 rounded-2xl p-4" style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                  <p className="text-xs font-bold text-zinc-400 uppercase tracking-wider mb-3">Recent Sessions</p>
                  <div className="space-y-2">
                    {sessions.slice(0, 5).map((s: any) => (
                      <div key={s.id} className="flex items-center gap-2 text-xs">
                        ...
                        <span className="text-zinc-600 truncate flex-1">{s.name ?? s.id.slice(0, 20)}</span>
