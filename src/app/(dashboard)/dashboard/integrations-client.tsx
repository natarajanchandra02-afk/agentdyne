 ...
 * Shows connected accounts + MCP registry inside the dashboard shell.
 * GPT recommendation: "Integrations should be a mini marketplace"
 */
...
import {
  ...
  Plus, Link2, Settings, AlertCircle,
} from "lucide-react"
...
// "Connected" demo state (would be persisted to DB in production)
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-4 gap-3">
        {[
          { label: "Total integrations",   value: MCP_SERVERS.length,                                  color: "text-primary"    },
          ...
          { label: "Quick setup (≤3 min)", value: MCP_SERVERS.filter(s => s.setupMinutes <= 3).length,  color: "text-amber-600"  },
        ].map(s => (
      ...
      <div className="flex items-center gap-3 flex-wrap">
        ...
        <div className="relative flex-1 min-w-[200px]">
          ...
          <input type="text" placeholder="Search integrations…" value={search}
            ...
            className="w-full pl-9 pr-4 h-9 rounded-xl border border-zinc-200 bg-white text-sm placeholder:text-zinc-400 focus:outline-none focus:border-zinc-400 focus:ring-2 focus:ring-zinc-100 transition-all" />
        </div>
              ...
              <div className={cn(
                ...
                <div className="flex items-start gap-3 mb-3">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-1.5 mb-1">
                      ...
                      {server.verified && <CheckCircle className="h-3.5 w-3.5 text-blue-500 flex-shrink-0" />}
                    </div>
                    <div className="flex items-center gap-2 flex-wrap">
                      <span className="text-[10px] text-zinc-400 flex items-center gap-0.5">
                        <Clock className="h-2.5 w-2.5" />{server.setupMinutes}min setup
                      </span>
                      {isConnected && (
                ...
                <div className="flex items-center gap-2 pt-3 border-t border-zinc-50">
                  <button type="button"
                    ...
                    className={cn(
                      "flex-1 h-8 text-xs gap-1.5 rounded-xl font-semibold flex items-center justify-center transition-all",
                      isConnected
