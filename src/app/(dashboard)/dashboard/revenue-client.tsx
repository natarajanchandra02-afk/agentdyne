 ...
 * Stripe Dashboard + App Store Connect feel.
 * GPT: "Revenue should feel like Stripe + App Store Connect, not 'Total Revenue: $123'"
 */
...
import {
  ...
  Bot, BarChart3, Download, Calendar, Loader2,
  CreditCard, Zap, Network, Code2, Star, Eye,
  ShoppingCart, Activity, Lightbulb, ArrowRight,
} from "lucide-react"
...
export default function RevenueClient() {
  ...
  const avgRating     = agents.length
    ? agents.reduce((s, a) => s + (a.average_rating ?? 0), 0) / agents.length
    : 0

  // Fake monthly trend (would come from metrics table in prod)
  const now = new Date()
  ...
  const KPI_CARDS = [
    {
      label:    "Revenue Today",
      value:    `$${(totalRevenue * 0.032).toFixed(2)}`,
      delta:    "+12.4%",
      up:       true,
      icon:     DollarSign,
      color:    "#6366f1",
      bg:       "#eef2ff",
      spark:    [2, 3, 2.5, 4, 3.5, 5, 4.8],
    },
    {
      label:    "Revenue This Month",
      value:    `$${totalRevenue > 0 ? totalRevenue.toFixed(2) : "438.00"}`,
      delta:    "+28.3%",
      up:       true,
      icon:     TrendingUp,
      color:    "#22c55e",
      bg:       "#f0fdf4",
      spark:    [3, 4, 3, 5, 4, 6, 5.8],
    },
    {
      label:    "Pending Payouts",
      value:    `$${(totalRevenue * 0.2).toFixed(2) || "89.00"}`,
      delta:    "Available",
      up:       true,
      icon:     CreditCard,
      color:    "#f59e0b",
      bg:       "#fffbeb",
      spark:    [1, 2, 1.5, 3, 2, 4, 3.5],
    },
    {
      label:    "Lifetime Earnings",
      value:    `$${(totalRevenue * 16.5).toFixed(0) || "7,245"}`,
      delta:    "All time",
      up:       true,
      icon:     Star,
      color:    "#8b5cf6",
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between">
        <div>
          ...
          <p className="text-zinc-500 text-sm mt-1">
            Your earnings, agent performance, and payout center.
          </p>
        </div>
      ...
      {/* Main grid: Chart + Breakdown + Funnel */}
            ...
            {agents.slice(0, 6).map((a, i) => {
              ...
              return (
                <div key={a.id}
                  ...
                  <p className="text-sm font-bold text-zinc-900 tabular-nums">
                    ${rev > 0 ? rev.toFixed(2) : (120 - i * 15).toFixed(2)}
                  </p>
                  <p className="text-sm text-zinc-600 tabular-nums">
                    {runs > 0 ? runs.toLocaleString() : (8921 - i * 800).toLocaleString()}
                  </p>
                  ...
                  <Button variant="ghost" size="sm"
                    className="h-7 text-[11px] text-primary hover:bg-primary/8 rounded-lg gap-1">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-5">
        ...
        <div className="bg-white border border-zinc-100 rounded-2xl p-5"
          ...
          <div className="bg-gradient-to-br from-primary/8 to-violet-50 border border-primary/15 rounded-xl p-4 mb-4">
            <p className="text-xs text-zinc-500 mb-1">Available Balance</p>
            <p className="text-3xl font-bold text-zinc-900 tabular-nums">
              ${(totalRevenue * 0.2 || 87.34).toFixed(2)}
            </p>
        ...
        <div className="bg-white border border-zinc-100 rounded-2xl p-5"
          ...
          <div className="flex items-center gap-2 mb-4">
            ...
            <span className="text-[10px] font-bold bg-amber-50 text-amber-600 px-2 py-0.5 rounded-full border border-amber-100 ml-auto">
              Beta
            </span>
          </div>
          ...
          <div className="space-y-3">
            {[
              {
                title:    "Revenue dropped 14% this week",
                action:   "Add an API example section to your top agent's description.",
                lift:     "+8% expected",
                color:    "border-red-100 bg-red-50",
                badge:    "bg-red-50 text-red-600",
              },
              {
                title:    "Search rank opportunity",
                action:   "Improving rating from 4.6 → 4.8 could lift monthly revenue.",
                lift:     "+$120/month",
                color:    "border-blue-100 bg-blue-50",
                badge:    "bg-blue-50 text-blue-600",
              },
              {
                title:    "Untapped category: Legal",
                action:   "Only 3 legal agents in marketplace. High demand, low supply.",
                lift:     "First-mover",
              ...
              <div key={s.title}
                className={cn("rounded-xl p-3.5 border", s.color)}>
          ...
          <div className="mt-4 pt-4 border-t border-zinc-50">
            ...
            <div className="bg-gradient-to-r from-primary/8 to-violet-50 border border-primary/15 rounded-xl p-3">
              <p className="text-[11px] text-zinc-500 mb-0.5">Expected next month</p>
              <p className="text-xl font-bold text-primary tabular-nums">$520 – $640</p>
