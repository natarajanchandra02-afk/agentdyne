                ...
                return (
                  <div key={agent.id}
                    ...
                    className={cn(
                      ...
                      added
                        ? "border-zinc-100 bg-zinc-50 opacity-50 cursor-default"
    ...
    <div className="space-y-3">
      ...
      {output && (
        <div className="space-y-1.5">
          ...
          <div className="min-h-[80px] max-h-[200px] overflow-auto rounded-xl border border-zinc-200 bg-zinc-50 font-mono text-xs p-3 whitespace-pre-wrap text-zinc-700">
            {output}
          </div>
        </div>
      )}
...
// ── Main Page — NO DashboardSidebar here; it comes from (dashboard)/layout.tsx ──
...
export default function PipelineEditPage() {
  ...
  const toggleContinue = (i: number) => setNodes(prev =>
    prev.map((n, j) => j === i ? { ...n, continue_on_failure: !n.continue_on_failure } : n)
  )
  const buildEdges = (ns: DAGNode[]) =>
  ...
  // NOTE: No DashboardSidebar here — (dashboard)/layout.tsx provides it.
  ...
  if (error || !pipeline) return (
    <div className="flex-1 flex items-center justify-center p-8 min-h-[60vh]">
      <div className="text-center">
        ...
        <Link href="/pipelines">
          <Button variant="outline" className="rounded-xl mt-3">← Back</Button>
        </Link>
      </div>
    </div>
  )
  ...
  return (
    // Just the page content — layout already provides sidebar + outer flex wrapper
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-zinc-400">
              {nodes.length} agent{nodes.length !== 1 ? "s" : ""} · Pipeline editor
            </p>
          </div>
        </div>
      ...
      {activeTab === "builder" && (
        <div className="grid grid-cols-1 lg:grid-cols-5 gap-5">
          ...
          <div className="lg:col-span-2 bg-white border border-zinc-100 rounded-2xl overflow-hidden flex flex-col"
            style={{ boxShadow: "0 1px 3px rgba(0,0,0,0.04)", maxHeight: 680 }}>
