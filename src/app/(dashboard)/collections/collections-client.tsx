 ...
 * Bug fixes:
 *  ✅ supabase client created once via useRef (not recreated on every render)
 *  ✅ loadCollections deps fixed — no infinite loop
 *  ✅ loading=false when user is null
 *  ✅ createCollection/updateCollection reset modal state correctly
 *  ✅ agent_count computed correctly from agent_ids length when col.agent_count is missing
 */
...
import {
  ...
  Check, X, MoreHorizontal, Lock, Globe,
...
interface Collection {
  ...
  agent_count: number; agent_ids: string[]
...
const COLOR_OPTIONS = ["#6366f1","#3b82f6","#22c55e","#f59e0b","#ef4444","#8b5cf6","#14b8a6","#f97316","#ec4899","#64748b"]
const STARTERS = [
  { name:"Marketing",       icon:"📣", color:"#ec4899", description:"Content, ads, and growth agents"  },
  { name:"Development",     icon:"⚡", color:"#3b82f6", description:"Code review, API design, testing"  },
  { name:"Finance",         icon:"💰", color:"#22c55e", description:"Analysis, invoicing, risk"          },
...
const ColourPicker = ({ value, onChange }:{ value:string; onChange:(c:string)=>void }) => (
  <div className="flex items-center gap-1.5 flex-wrap">
    {COLOR_OPTIONS.map(c=>(
      <button key={c} type="button" onClick={()=>onChange(c)}
        className="w-6 h-6 rounded-full flex items-center justify-center transition-transform hover:scale-110"
        style={{ background:c }}>
        {value===c&&<Check style={{ width:11,height:11,color:"#fff",strokeWidth:3 }}/>}
      </button>
    ))}
  </div>
)
...
const IconPicker = ({ value, onChange }:{ value:string; onChange:(i:string)=>void }) => (
    ...
    {ICON_OPTIONS.map(ic=>(
      <button key={ic} type="button" onClick={()=>onChange(ic)}
        className={cn("w-8 h-8 rounded-lg text-base flex items-center justify-center transition-all",
          value===ic?"bg-zinc-900 scale-110":"bg-zinc-100 hover:bg-zinc-200")}>
        {ic}
      </button>
    ))}
  </div>
...
function CollectionModal({ open, onClose, onSave, initial }:{
  ...
  onSave:(d:Pick<Collection,"name"|"description"|"icon"|"color"|"is_public">)=>Promise<void>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4"
          ...
          <motion.div initial={{ opacity:0,scale:0.96,y:8 }} animate={{ opacity:1,scale:1,y:0 }}
            ...
            <div className="flex items-center justify-between px-5 py-4 border-b border-zinc-100">
              <p className="text-sm font-bold text-zinc-900">{initial?.id?"Edit collection":"New collection"}</p>
            ...
            <div className="p-5 space-y-4">
              ...
              <div>
                ...
                <textarea value={desc} onChange={e=>setDesc(e.target.value)} rows={2}
                  ...
                  className="w-full rounded-xl border border-zinc-200 px-3 py-2 text-sm resize-none focus:outline-none"
              ...
              <div className="flex items-center gap-2.5">
                <button type="button" onClick={()=>setIsPublic(v=>!v)}
                  ...
                  style={{ width:36,height:20,background:isPublic?color:"#d4d4d8",flexShrink:0 }}>
                  <span className="absolute top-[2px] rounded-full bg-white shadow-sm transition-transform duration-200"
                    style={{ width:16,height:16,transform:isPublic?"translateX(18px)":"translateX(2px)" }}/>
                </button>
                <span className="text-xs text-zinc-600 font-medium flex items-center gap-1.5">
                  {isPublic
                    ?<><Globe style={{ width:11,height:11 }}/>Public — visible to marketplace</>
                    :<><Lock style={{ width:11,height:11 }}/>Private — only you</>}
                </span>
              </div>
            </div>
            {/* Footer */}
            <div className="flex gap-2.5 px-5 pb-5">
              ...
              <button onClick={handle} disabled={saving||!name.trim()}
                ...
                style={{ background:name.trim()?color:`${color}55`, cursor:name.trim()?"pointer":"default" }}>
...
function CollectionCard({ col, onEdit, onDelete, agents }:{
  ...
  // Bug fix: compute agent count from agent_ids if agent_count is 0/null
  const agentIds   = col.agent_ids ?? []
  const agentCount = col.agent_count ?? agentIds.length
  const colAgents  = agents.filter(a => agentIds.includes(a.id))
  ...
  return (
    <motion.div layout
      ...
      {/* Top colour bar */}
      <div className="h-1.5 w-full" style={{ background:`linear-gradient(90deg,${col.color},${col.color}80)` }}/>
      <div className="p-4">
        <div className="flex items-start justify-between gap-2 mb-3">
          ...
          <div className="flex items-center gap-2.5 min-w-0 flex-1">
            ...
            <div className="min-w-0">
              ...
              {col.description&&<p className="text-[11px] text-zinc-400 truncate mt-0.5">{col.description}</p>}
            </div>
          </div>
          {/* 3-dot menu */}
          <div className="relative flex-shrink-0">
            <button type="button" onClick={()=>setMenuOpen(v=>!v)}
              className="w-7 h-7 rounded-lg flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity hover:bg-zinc-100">
              ...
              {menuOpen&&(
                <>
                  ...
                  <motion.div initial={{ opacity:0,scale:0.95,y:-4 }} animate={{ opacity:1,scale:1,y:0 }}
                    ...
                    className="absolute right-0 top-full mt-1 w-36 bg-white border border-zinc-100 rounded-xl shadow-lg z-20 py-1 overflow-hidden">
        ...
        <div className="flex items-center justify-between">
          ...
          <div className="flex items-center gap-2">
            {col.is_public
              ?<Badge variant="secondary" className="text-[10px] h-5 px-1.5 gap-0.5"><Globe style={{ width:8,height:8 }}/> Public</Badge>
              :<Badge variant="secondary" className="text-[10px] h-5 px-1.5 gap-0.5 bg-zinc-50 text-zinc-400"><Lock style={{ width:8,height:8 }}/> Private</Badge>}
            <span className="text-xs font-semibold text-zinc-500">{agentCount} agent{agentCount!==1?"s":""}</span>
          </div>
        </div>
      </div>
      {/* Open link */}
      <Link href={`/collections/${col.id}`}>
        <div className="flex items-center gap-1 px-4 py-2.5 border-t border-zinc-50 text-xs font-semibold text-zinc-400 hover:text-zinc-700 hover:bg-zinc-50 transition-all">
          Open collection <ChevronRight style={{ width:12,height:12 }}/>
        </div>
      </Link>
    </motion.div>
  )
...
export default function CollectionsClient() {
  ...
  // Bug fix: create supabase once, not on every render
  ...
  // Bug fix: loadCollections no longer has supabase in deps (it's stable via ref)
  const loadCollections = useCallback(async () => {
    ...
    setLoading(false)
  // supabase ref is stable — safe to omit
  // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [user?.id])

  useEffect(()=>{
    // Bug fix: stop loading immediately when user is null
  ...
  const createStarters = async () => {
    ...
    const rows = STARTERS.map((s,i)=>({
      ...
      icon:s.icon, color:s.color, is_public:false, agent_ids:[], sort_order:i,
    }))
  ...
  const unorganised = agents.filter(a=>!collections.some(c=>(c.agent_ids??[]).includes(a.id))).length
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-start justify-between gap-4 flex-wrap">
        <div>
          ...
          <p className="text-zinc-500 text-sm mt-1">Organise your agents into folders — like Pinterest boards for AI.</p>
        </div>
        <div className="flex items-center gap-2">
      ...
      {collections.length>0&&(
        <div className="grid grid-cols-3 gap-3">
          {[
            {label:"Collections",     value:collections.length,                                    color:"text-indigo-600"},
            {label:"Agents organised",value:collections.reduce((s,c)=>s+(c.agent_count??0),0),    color:"text-green-600"},
            {label:"Public",          value:collections.filter(c=>c.is_public).length,             color:"text-blue-600"},
          ].map(s=>(
      ...
      {collections.length>0&&(
        <div className="relative max-w-sm">
          ...
          <input type="text" placeholder="Search collections…" value={search}
            ...
            className="w-full pl-9 pr-4 h-9 rounded-xl border border-zinc-200 bg-white text-sm focus:outline-none focus:border-zinc-400 transition-all"/>
        </div>
      )}
      ...
      {!loading&&agents.length>0&&unorganised>0&&(
        <div className="bg-amber-50 border border-amber-100 rounded-2xl p-4 flex items-center gap-3">
          <Bot className="h-5 w-5 text-amber-500 flex-shrink-0"/>
          <div className="flex-1 min-w-0">
            <p className="text-sm font-semibold text-amber-900">
              {unorganised} agent{unorganised!==1?"s":""} not in any collection
            </p>
            <p className="text-xs text-amber-700 mt-0.5">
              Add them to a collection to keep things tidy.
            </p>
          </div>
          <Link href="/my-agents">
