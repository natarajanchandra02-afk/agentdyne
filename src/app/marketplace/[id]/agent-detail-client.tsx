...
const SAMPLE_DATA: Record<string, { label: string; json: string }> = {
  customer_support: { label: "Customer complaint",    json: JSON.stringify({ input: "My order #87234 hasn't arrived after 2 weeks. The tracking just says 'In Transit'. I need this urgently for a birthday event this weekend." }, null, 2) },
  coding:           { label: "Code review",          json: JSON.stringify({ input: "Review this Python function:\n\ndef calculate_discount(price, pct):\n    return price - (price * pct)\n\nprint(calculate_discount(100, 20))" }, null, 2) },
  marketing:        { label: "Social post",          json: JSON.stringify({ input: "Write a LinkedIn post announcing our new AI analytics dashboard. Target: B2B SaaS founders. Tone: professional but approachable. Include a CTA." }, null, 2) },
  finance:          { label: "P&L analysis",         json: JSON.stringify({ input: "Analyze: Revenue Q1=$2.1M, Q2=$2.4M, Q3=$2.2M, Q4=$3.1M. COGS: 40%. OpEx: $800K/quarter. Provide key insights." }, null, 2) },
  legal:            { label: "Contract review",      json: JSON.stringify({ input: "Review: 'The receiving party agrees to keep all disclosed information confidential for 2 years from disclosure.' Identify risks." }, null, 2) },
  data_analysis:    { label: "Sales trend",          json: JSON.stringify({ input: "Analyze: Jan=142, Feb=156, Mar=198, Apr=187, May=203, Jun=245. Identify trend, MoM growth, forecast Jul-Sep." }, null, 2) },
  content:          { label: "Blog intro",           json: JSON.stringify({ input: "Write a 500-word blog intro about 'Why microagents are the future of AI automation'. Target: startup CTOs." }, null, 2) },
  research:         { label: "Research brief",       json: JSON.stringify({ input: "Summarise the state of autonomous AI agents in 2025: key capabilities, limitations, and 3 most significant breakthroughs." }, null, 2) },
  security:         { label: "Security audit",       json: JSON.stringify({ input: "Review for vulnerabilities:\n\ndef login(u, p):\n    q = f\"SELECT * FROM users WHERE username='{u}' AND password='{p}'\"\n    r = db.execute(q)\n    if r: return create_session(r[0]['id'])" }, null, 2) },
  _default:         { label: "Sample input",         json: JSON.stringify({ input: "Hello! Can you show me what you can do? Give me a brief demonstration." }, null, 2) },
}
  ...
  return (
    <div className="bg-white border border-zinc-100 rounded-2xl p-5">
      ...
      <Link href={`/marketplace?category=${category}`}>
        <p className="text-xs text-primary font-semibold mt-3 hover:underline">
...
function AddToCollectionModal({ agentId, onClose }: { agentId: string; onClose: () => void }) {
  ...
  const add = async (collectionId: string, currentIds: string[]) => {
    if (currentIds.includes(agentId)) { toast.error("Already in this collection"); return }
    setAdding(collectionId)
    const { error } = await supabase
      .from("collections")
      .update({ agent_ids: [...currentIds, agentId] })
      .eq("id", collectionId)
        ...
        <div className="p-4 space-y-2 max-h-64 overflow-y-auto">
          {loading ? (
            ...
            <div className="text-center py-6 text-sm text-zinc-400">
              No collections yet.{" "}
              <Link href="/collections" className="text-primary hover:underline text-xs mt-1 block">
                Create one →
              </Link>
            </div>
          ) : collections.map(c => {
            const already = (c.agent_ids || []).includes(agentId)
            return (
              <button key={c.id} disabled={already || adding === c.id}
                onClick={() => add(c.id, c.agent_ids || [])}
                className={cn(
                ...
                {already ? <Check className="h-4 w-4 text-green-500" />
...
function ReportModal({ agentId, agentName, onClose }: {
  agentId: string; agentName: string; onClose: () => void
}) {
  const REASONS = [
    "Broken / not working",
    "Inaccurate or misleading description",
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl border border-zinc-100 w-full max-w-sm z-10 p-5">
        {sent ? (
          <div className="text-center py-4">
            ...
            <p className="text-sm text-zinc-400 mb-4">Our team will review this agent.</p>
          ...
          <>
            <div className="flex items-center justify-between mb-4">
              <p className="text-sm font-semibold text-zinc-900 flex items-center gap-2">
                <Flag className="h-4 w-4 text-red-500" /> Report Agent
              </p>
              <button onClick={onClose} className="p-1.5 rounded-lg text-zinc-400 hover:text-zinc-700 hover:bg-zinc-50">
                <X className="h-4 w-4" />
              </button>
            ...
            <div className="space-y-2 mb-3">
              {REASONS.map(r => (
                <button key={r} onClick={() => setReason(r)}
                  className={cn(
                    "w-full flex items-center gap-2 px-3 py-2 rounded-xl text-sm border text-left transition-all",
                    reason === r
...
function ReviewForm({ agentId, onReviewSubmitted }: {
  agentId: string; onReviewSubmitted: (r: any) => void
}) {
  ...
  return (
    <div className="bg-white border border-zinc-100 rounded-2xl p-5 space-y-4">
      ...
      <p className="text-[11px] text-zinc-400">
        You must have run this agent at least once. Reviews are moderated before appearing publicly.
      </p>
      <Button
        onClick={submit}
...
export function AgentDetailClient({ agent, reviews: initReviews, user, userSubscription, isOwner }: Props) {
  ...
  const [traceInfo,   setTraceInfo]   = useState<{
    latencyMs: number; cost: number; tokens?: { input: number; output: number }
  } | null>(null)
  ...
  const loadMoreReviews = async () => {
    ...
    try {
      const res  = await fetch(`/api/agents/${agent.id}/reviews?page=${next}&limit=10`)
      const data = await res.json()
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {showReport && (
        <ReportModal agentId={agent.id} agentName={agent.name} onClose={() => setShowReport(false)} />
      )}
      {showCollect && user && (
        <AddToCollectionModal agentId={agent.id} onClose={() => setShowCollect(false)} />
      )}
      ...
      <div className="pt-14">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          {/* Bug 10 FIX: draft/pending banner for owner */}
          {isOwner && agent.status !== "active" && (
            <div className={cn(
              ...
              <AlertTriangle className={cn(
                ...
                agent.status === "pending_review" ? "text-amber-500" : "text-zinc-400"
              )} />
              <div className="flex-1">
                <p className={cn(
                  "text-sm font-semibold",
                  agent.status === "pending_review" ? "text-amber-800" : "text-zinc-700"
                )}>
                  {agent.status === "pending_review" ? "This agent is under review" : "This agent is a draft"}
                </p>
                <p className={cn(
                  "text-xs mt-0.5",
                  agent.status === "pending_review" ? "text-amber-600" : "text-zinc-500"
                )}>
                  {agent.status === "pending_review"
                    ? "Not visible to others yet. Our team reviews submissions within 24 hours."
                    : "Only visible to you. Complete the setup and submit for review to go live."}
                </p>
              </div>
              <Link href={`/builder/${agent.id}`}>
                <Button size="sm" variant="outline" className={cn(
                  "rounded-xl flex-shrink-0 text-xs",
                  agent.status === "pending_review"
            ...
            <div className="lg:col-span-2 space-y-6">
              ...
              <div className="flex items-start gap-4">
                ...
                <div className="flex-1">
                  ...
                  {/* Bug 13 FIX: render long_description */}
                  ...
                  <div className="flex items-center gap-4 mt-3 flex-wrap text-sm">
                    <span className="flex items-center gap-1">
                      ...
                      {/* Bug 12 FIX: approved count, not denormalized total */}
              ...
              {/* Tabs — single Radix implementation, no custom state */}
              <Tabs defaultValue="playground">
                ...
                {/* ── Playground — Bug 16 FIX: responsive (stacked mobile, 2-col md+) */}
                <TabsContent value="playground" className="mt-4 space-y-4">
                  ...
                  {traceInfo && (
                    <div className="bg-zinc-50 border border-zinc-100 rounded-xl px-4 py-2.5 flex flex-wrap items-center gap-4 text-xs text-zinc-500">
                      <span className="flex items-center gap-1">
                        <Clock className="h-3 w-3" /> {traceInfo.latencyMs}ms
                      </span>
                      {traceInfo.tokens && (
                        <span className="flex items-center gap-1">
                          <BarChart3 className="h-3 w-3" />
                          {traceInfo.tokens.input}↑ {traceInfo.tokens.output}↓ tokens
                        </span>
                      )}
...
{`// window.location.origin makes this portable across environments
...
const { executionId, output, latencyMs, cost } = await res.json();`}
                ...
                <TabsContent value="reviews" className="mt-4 space-y-4">
                  {/* Bug 5 FIX: review form for authenticated users */}
                  {user && (
                    <ReviewForm agentId={agent.id} onReviewSubmitted={() => {}} />
                  )}
                  ...
                  {reviews.length === 0 ? (
                    <div className="text-center py-10 text-zinc-400 text-sm bg-white border border-zinc-100 rounded-2xl">
                      ...
                      {!user
                        ? <> <Link href="/login" className="text-primary hover:underline">Sign in</Link> to be the first.</>
                        : " Run this agent and leave the first review!"}
                    </div>
                  ) : (
                    <>
                      {reviews.map(r => (
                              ...
                              <div className="flex gap-0.5 mb-2">
                                {[...Array(5)].map((_, i) => (
                                  <Star key={i} className={cn(
                                    "h-3 w-3",
                                    i < r.rating ? "fill-yellow-400 text-yellow-400" : "text-zinc-200"
                                  )} />
                                ))}
                              </div>
                              {r.title && (
            ...
            <div className="space-y-4">
              <div
                ...
                <Button
                  variant="outline"
                  className="w-full rounded-xl border-zinc-200 font-semibold text-zinc-700 gap-2 mb-3"
                  onClick={() => user
                ...
                {user && (
                  <Button
                    variant="outline"
                    className="w-full rounded-xl border-zinc-200 font-semibold text-zinc-600 gap-2 mb-2 text-sm"
                ...
                {agent.free_calls_per_month > 0 && (
                  <p className="text-center text-xs text-zinc-400 mt-1 mb-3">
              ...
              {/* Seller card — Bug 9 FIX: total_earned NOT in query */}
              <div className="bg-white border border-zinc-100 rounded-2xl p-5">
                <h3 className="text-xs font-semibold text-zinc-500 uppercase tracking-wider mb-3">
                  About the Builder
                </h3>
