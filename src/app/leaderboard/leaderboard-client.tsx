...
import {
  ...
  ChevronDown, BarChart3, BadgeCheck, Filter,
  Minus, AlertTriangle, Info,
} from "lucide-react"
...
function ScoreBar({ score, color = "bg-primary" }: { score: number; color?: string }) {
  return (
    <div className="flex items-center gap-2 w-full">
      ...
      <span className="text-xs font-mono text-zinc-500 w-8 text-right tabular-nums">
        {(score ?? 0).toFixed(0)}
      </span>
    </div>
  )
...
function ConfidencePill({ confidence }: { confidence: number }) {
  ...
  return (
    <span
      title={`Based on ${c.toFixed(0)}% of ideal execution volume`}
      ...
      <div className="pt-14">
        ...
        <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-8">
          ...
          {loading ? (
            ...
            <div className="space-y-2">
              {agents.map((agent, i) => {
                ...
                const isPaid = (agent.pricing_model ?? "free") !== "free"
                  ...
                  <div key={agent.id}
                    ...
                    <div className="flex items-center gap-3 sm:gap-4 px-4 sm:px-5 py-4">
                      ...
                      <div className="hidden lg:flex items-center gap-4 flex-shrink-0">
                        ...
                        <div className="text-center">
                          <div className="text-sm font-bold text-zinc-900 tabular-nums">{agent.average_latency_ms}ms</div>
                          <LatencyPill label={agent.latency_label ?? "—"} color={agent.latency_color ?? "zinc"} />
                        </div>
                        <div className="text-center">
                          <div className={cn(
                            "text-sm font-bold tabular-nums",
                            (agent.failure_rate ?? 0) > 20 ? "text-red-500" :
                            (agent.failure_rate ?? 0) > 5  ? "text-amber-500" : "text-green-600"
                          )}>
                        ...
                        <div className="flex flex-wrap items-center gap-4 pt-1 border-t border-zinc-50">
                          <div className="flex items-center gap-1.5">
                            <AlertTriangle className={cn(
                              "h-3.5 w-3.5",
                              (agent.failure_rate ?? 0) > 20 ? "text-red-400" : "text-zinc-300"
                            )} />
                            <span className="text-xs text-zinc-500">
                              Failure rate:{" "}
                              <strong className={(agent.failure_rate ?? 0) > 20 ? "text-red-500" : "text-zinc-700"}>
                                {(agent.failure_rate ?? 0).toFixed(1)}%
                              </strong>
                            </span>
                          </div>
                          <div className="flex items-center gap-1.5">
                            ...
                            <span className="text-[10px] text-zinc-400">
                              ({formatNumber(agent.total_executions)} verified runs)
                            </span>
                          </div>
          ...
          <div className="flex flex-col sm:flex-row items-center justify-between gap-2 mt-8">
            ...
            <p className="text-xs text-zinc-400">Cheapest badge requires Quality ≥ 60/100</p>
          </div>
        </div>
      </div>
      <Footer />
